...
In our previous lessons on the Brown capstone project, we introduced its design and architecture in Lessons 20 and 21. We then moved from theory to practice, implementing the core writing workflow and adding an automated review loop to enforce quality.
...
```
Image 1: Inverse relationship between an AI agent's level of control and application reliability. (Source: [Decoding ML](https://decodingml.substack.com/p/llmops-for-production-agentic-rag))
...
```
Image 2: A flowchart illustrating the iterative AI generation and human validation loop. (Source: [Decoding ML](https://decodingml.substack.com/p/llmops-for-production-agentic-rag))
...
5.  Let's see a practical example. We start by importing the necessary components and loading our sample article, guideline, and profiles.
    ...
    def parse_message(message: str, ctx: ToolContext) -> None:
        if "{" in message and "}" in message:
            try:
                ...
                progress = WorkflowProgress.model_validate(data)
                ctx.report_progress(progress.progress / 100, progress.message)
...
5.  We also define three prompts that make it easy for users to trigger the tools without reading documentation.
    ...
    def generate_article_prompt(dir_path: Path) -> str:
        """Generate an article from scratch."""
        return f"Generate an article from the files in the following directory: {dir_path}"
    ...
    def edit_article_prompt(human_feedback: str | None = None) -> str:
        ...
        if human_feedback:
            return f"Edit the article with the following feedback: {human_feedback}"
        return "Edit the article."
    
    
    @mcp.prompt()
    def edit_selected_text_prompt(human_feedback: str | None = None) -> str:
        """Edit a selected section of an article."""
        if human_feedback:
            return f"Edit the selected text with the following feedback: {human_feedback}"
        return "Edit the selected text."
    ```
...
6.  Finally, we expose two resources that provide read-only access to configuration and state.
    ...
    def get_character_profile() -> str:
        ...
        profiles_loader = MarkdownArticleProfilesLoader(
            uri={
                "character": PROFILES_DIR / "character_profiles" / "paul_iusztin.md",
            }
        )
        profiles = profiles_loader.load()
        return profiles.character.content
    ```
...
You can further extend the code from this lesson in several ways:
...
With this lesson, we have wrapped up our exploration of the Brown writing workflow. We covered its foundation with entities and nodes in Lesson 22, the evaluator-optimizer pattern in Lesson 23, and now, human-in-the-loop with MCP server integration.

In Part 2D, we will explore how to orchestrate Nova and Brown as a multi-agent system within a single MCP client, automating the whole research-to-article workflow. Then in Part 3, we will cover production engineering practices like AI Evals, monitoring, deployment, and more.
