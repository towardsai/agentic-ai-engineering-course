...
AI-generated results are imperfect. LLMs can hallucinate, display "jagged intelligence" by being superhuman at one task and failing at a simpler one, and make mistakes no human would [[35]](https://www.youtube.com/shorts/lZZZjaQepHI). This creates the need for a balance between AI output and human input. You must cooperate with the AI, where you do most of the thinking while delegating the monotonous work to the AI. Your role is to plan, give instructions, and validate the AI’s output, not to delegate the thinking itself.
...
Andrej Karpathy argues that the goal is to make this loop spin as fast as possible to maximize productivity [[32]](https://www.latent.space/p/s3), [[33]](https://catalaize.substack.com/p/andrej-karpathy-software-is-changing). He outlines two key strategies for this.
...
```
Image 1: Inverse relationship between an AI agent's level of control and application reliability.

First, you must speed up verification. Application-specific GUIs help humans audit AI work faster by using visual representations. For example, showing code diffs in red and green reduces cognitive load compared to reading raw text. Direct actions, like accepting or rejecting changes with a simple command, are far more efficient than typing out new instructions [[32]](https://www.latent.space/p/s3).

Second, you need to keep the AI on a tight leash. An AI can be overreactive; receiving a 10,000-line code diff is not useful because a human must still verify it. It is better to work in small, incremental chunks focused on a "single concrete thing" and use concrete prompts to increase the probability of a successful verification [[33]](https://catalaize.substack.com/p/andrej-karpathy-software-is-changing).

This connects directly to the autonomy slider we discussed in Lesson 2. As you give an application more autonomy, you reduce the need for human intervention. Karpathy suggests building products that are "less Iron Man robots and more Iron Man suits," focusing on human augmentation rather than full autonomy [[32]](https://www.latent.space/p/s3).
...
```
Image 2: A flowchart illustrating the iterative AI generation and human validation loop.
...
```mermaid
graph TD
    A["Human Input"] --> B["Generate Article (MCP Tool)"]
    B --> C{"Human Review & Feedback"}
    C --> D["Edit Article (MCP Tool)"]
    C --> E["Edit Selected Text (MCP Tool)"]
    D --> C
    E --> C
    C --> F["End (Satisfied)"]
```
Image 3: A flowchart illustrating the Brown writing workflow with human-in-the-loop, showing three independent MCP tools and a human feedback loop.
...
The `edit_article` workflow reviews and edits an existing article based on human feedback and predefined requirements. It consists of a single loop of the reviewing-editing logic we used in the `generate_article` workflow. It follows the same clean architecture, leveraging the app layer to orchestrate nodes and entities. Because our entities and nodes are orthogonal, we can reuse them as-is; we only need to implement the glue code in the app layer to wrap them for this new business use case.
...
5.  This workflow is crucial because most of the time, we do not want to edit the whole article, but just a small section. It enables faster, cheaper, and more precise changes, giving you better control over the editing process.
...
To integrate with Cursor, you configure the MCP server in the `.cursor/mcp.json` file. This tells Cursor the server's name, how to launch it, and where to find its working directory and environment variables.
...
As proof of our process, we generated this very lesson using the exact methods we have explained here—a real-world demonstration of the human-in-the-loop writing process.
...
You can extend this project by hooking Brown to other clients like Claude Desktop, parameterizing the writing profiles as resources, or even serving Brown through a FastAPI for web integrations.
...
1.  Zapier. (n.d.). *What is human-in-the-loop?* Zapier. https://zapier.com/blog/human-in-the-loop/
2.  WorkOS. (n.d.). *Why AI still needs you: Exploring human-in-the-loop systems*. WorkOS. https://workos.com/blog/why-ai-still-needs-you-exploring-human-in-the-loop-systems
3.  WitnessAI. (n.d.). *Human-in-the-loop (HITL) AI: A comprehensive guide*. WitnessAI. https://witness.ai/blog/human-in-the-loop-ai/
4.  metaphacts. (n.d.). *Human-in-the-loop for AI: A collaborative future in research workflows*. metaphacts. https://blog.metaphacts.com/human-in-the-loop-for-ai-a-collaborative-future-in-research-workflows
5.  Davis, C. (n.d.). *Human-in-the-loop for AI agents*. YouTube. https://www.youtube.com/watch?v=YCFGjLjNOyw
6.  Permit.io. (n.d.). *Human-in-the-loop for AI agents: Best practices, frameworks, use cases, and demo*. Permit.io. https://www.permit.io/blog/human-in-the-loop-for-ai-agents-best-practices-frameworks-use-cases-and-demo
7.  IBM. (n.d.). *Build a human-in-the-loop AI agent with LangGraph and watsonx.ai*. IBM. https://www.ibm.com/think/tutorials/human-in-the-loop-ai-agent-langraph-watsonx-ai
8.  Orkes. (n.d.). *Human-in-the-loop in agentic AI workflows*. Orkes. https://orkes.io/blog/human-in-the-loop/
9.  Cursor. (n.d.). *Agent overview*. Cursor. https://cursor.com/docs/agent/overview
10. Cursor. (n.d.). *Features*. Cursor. https://cursor.com/features
11. DataCamp. (n.d.). *Cursor AI code editor tutorial*. DataCamp. https://www.datacamp.com/tutorial/cursor-ai-code-editor
12. Young, A. (n.d.). *How to use Cursor AI*. Alexander F. Young. https://blog.alexanderfyoung.com/how-to-use-cursor-ai/
13. AWS. (n.d.). *Serverless generative AI architectural patterns - Part 2*. AWS. https://aws.amazon.com/blogs/compute/part-2-serverless-generative-ai-architectural-patterns/
14. Carpintero, D. (n.d.). *Design patterns for building agentic workflows*. Hugging Face. https://huggingface.co/blog/dcarpintero/design-patterns-for-building-agentic-workflows
15. Emergent Mind. (n.d.). *Decoupled Explanation API*. Emergent Mind. https://www.emergentmind.com/topics/decoupled-explanation-api
16. Prompts.ai. (n.d.). *Decoupled AI pipelines & dependency management best practices*. Prompts.ai. https://www.prompts.ai/en/blog/decoupled-ai-pipelines-dependency-management-best-practices
17. paella doc. (n.d.). *Stop guessing: 5 AI architecture patterns*. paella doc. https://paelladoc.com/blog/stop-guessing-5-ai-architecture-patterns/
18. Catio. (n.d.). *Emerging architecture patterns for the AI-native enterprise*. Catio. https://www.catio.tech/blog/emerging-architecture-patterns-for-the-ai-native-enterprise
19. UXness. (2024, June). *7 principles of UX design for AI-driven products*. UXness. https://www.uxness.in/2024/06/7-principles-of-ux-design-for.html
20. Aufait UX. (n.d.). *10 usability principles for designing AI interfaces*. Aufait UX. https://www.aufaitux.com/blog/ai-interface-usability-principles/
21. Eleken. (n.d.). *9 AI usability principles to follow*. Eleken. https://www.eleken.co/blog-posts/ai-usability-principles
22. UX studio. (n.d.). *10 AI UX principles*. UX studio. https://www.uxstudioteam.com/ux-blog/ai-ux-5f836
23. Hatchet. (n.d.). *Evaluator-optimizer*. Icepick. https://icepick.hatchet.run/patterns/evaluator-optimizer
24. PraisonAI. (n.d.). *Agentic Evaluator-Optimizer*. PraisonAI. https://docs.praison.ai/docs/features/evaluator-optimiser
25. Spring. (2025, January 21). *Building effective agents: Spring AI agentic patterns*. Spring. https://spring.io/blog/2025/01/21/spring-ai-agentic-patterns
26. Java AI Dev. (n.d.). *Evaluator-Optimizer*. Java AI Dev. https://javaaidev.com/docs/agentic-patterns/patterns/evaluator-optimizer
27. BootcampToProd. (n.d.). *spring-ai-evaluator-optimizer-workflow*. GitHub. https://github.com/BootcampToProd/spring-ai-evaluator-optimizer-workflow
28. Roach, C. (n.d.). *Building self-correcting LLM systems: The evaluator-optimizer pattern*. DEV Community. https://dev.to/clayroach/building-self-correcting-llm-systems-the-evaluator-optimizer-pattern-169p
29. AWS. (n.d.). *Workflow for evaluators and reflect-refine loops*. AWS Prescriptive Guidance. https://docs.aws.amazon.com/prescriptive-guidance/latest/agentic-ai-patterns/workflow-for-evaluators-and-reflect-refine-loops.html
30. Castillo, D. (n.d.). *Evaluator-optimizer with Pydantic AI*. Dylan Castillo. https://dylancastillo.co/til/evaluator-optimizer-pydantic-ai.html
31. Latent Space. (n.d.). *Software 3.0*. Latent Space. https://www.latent.space/p/s3
32. Catalaize. (n.d.). *Andrej Karpathy: Software is changing*. Catalaize. https://catalaize.substack.com/p/andrej-karpathy-software-is-changing
33. YouTube. (n.d.). *Andrej Karpathy on human-in-the-loop*. YouTube. https://www.youtube.com/shorts/hTT5o2AoewQ
34. YouTube. (n.d.). *Andrej Karpathy on "jagged intelligence"*. YouTube. https://www.youtube.com/shorts/lZZZjaQepHI
35. Code Conductor. (n.d.). *How to build an AI project that doesn’t fail*. Code Conductor. https://codeconductor.ai/blog/build-ai-project-that-dont-fail/
36. AIPM Guru. (n.d.). *AI architecture patterns 101: Workflows, agents, and MCP*. AIPM Guru. https://aipmguru.substack.com/p/ai-architecture-patterns-101-workflows
37. uibakery.io. (n.d.). *Claude Code vs Cursor*. uibakery.io. https://uibakery.io/blog/claude-code-vs-cursor
38. Graphite. (n.d.). *Programming with AI workflows: Claude, Copilot, and Cursor*. Graphite. https://graphite.com/guides/programming-with-ai-workflows-claude-copilot-cursor
39. PAIR. (n.d.). *Design AI feedback loops*. People + AI Research (PAIR). https://pair.withgoogle.com/guidebook/chapters/feedback-and-controls/design-ai-feedback-loops
40. Becoming Human. (n.d.). *UX design for implicit and explicit feedback in an AI product*. Becoming Human. https://becominghuman.ai/ux-design-for-implicit-and-explicit-feedback-in-an-ai-product-9497dce737ea
41. UX Design.cc. (n.d.). *UX design principles for AI products*. UX Design.cc. https://uxdesign.cc/ux-design-principles-for-ai-products-8989aa55819d
42. Tentackles. (n.d.). *AI-first UX/UI principles*. Tentackles. https://tentackles.com/blog/ai-first-ux-ui-principles-zero-click
43. Anthropic. (n.d.). *Building effective agents*. Anthropic. https://www.anthropic.com/research/building-effective-agents
44. AWS. (n.d.). *Evaluator reflect-refine loop patterns*. AWS Prescriptive Guidance. https://docs.aws.amazon.com/prescriptive-guidance/latest/agentic-ai-patterns/evaluator-reflect-refine-loop-patterns.html
45. ASAPP. (n.d.). *Designing APIs for GenerativeAgent*. ASAPP. https://docs.asapp.com/generativeagent/configuring/connect-apis/designing-apis-for-generativeagent
46. Databricks. (n.d.). *Generative AI developer workflow*. Databricks. https://docs.databricks.com/aws/en/generative-ai/tutorials/ai-cookbook/genai-developer-workflow
47. AWS. (n.d.). *Best practices to build generative AI applications on AWS*. AWS. https://aws.amazon.com/blogs/machine-learning/best-practices-to-build-generative-ai-applications-on-aws/
48. Nordic APIs. (n.d.). *How to design robust generative AI APIs*. Nordic APIs. https://nordicapis.com/how-to-design-robust-generative-ai-apis/
49. Google. (n.d.). *Gemini Canvas overview*. Gemini. https://gemini.google/overview/canvas/
50. Canva. (n.d.). *Canva AI assistant*. Canva. https://www.canva.com/ai-assistant/
51. JetBrains. (n.d.). *JetBrains AI Assistant*. JetBrains. https://www.jetbrains.com/ai-assistant/
52. Google. (n.d.). *Gemini Code Assist*. Google. https://codeassist.google
53. Augment Code. (n.d.). *Augment Code*. Augment Code. https://www.augmentcode.com
54. AI Patterns. (n.d.). *AI patterns for document editors*. AI Patterns. https://aipatterns.substack.com/p/ai-patterns-for-document-editors
55. Koru UX. (n.d.). *AI patterns for UI design*. Koru UX. https://www.koruux.com/ai-patterns-for-ui-design/
56. Userpilot. (n.d.). *AI UX design*. Userpilot. https://userpilot.com/blog/ai-ux-design/
57. UX studio. (n.d.). *AI design patterns in SaaS products*. UX studio. https://www.uxstudioteam.com/ux-blog/ai-design-patterns-in-saas-products
58. Aufait UX. (n.d.). *Agentic AI design patterns: An enterprise guide*. Aufait UX. https://www.aufaitux.com/blog/agentic-ai-design-patterns-enterprise-guide/
59. UX Design.cc. (n.d.). *20 GenAI UX patterns*. UX Design.cc. https://uxdesign.cc/20-genai-ux-patterns-examples-and-implementation-tactics-5b1868b7d4a1
60. Google Cloud. (n.d.). *Human-in-the-loop*. Google Cloud. https://cloud.google.com/discover/human-in-the-loop
61. Tredence. (n.d.). *Human-in-the-loop (HITL)*. Tredence. https://www.tredence.com/blog/hitl-human-in-the-loop
62. Parseur. (n.d.). *Human-in-the-loop AI*. Parseur. https://parseur.com/blog/human-in-the-loop-ai
63. Digital Divide Data. (n.d.). *Human-in-the-loop for generative AI*. Digital Divide Data. https://www.digitaldividedata.com/blog/human-in-the-loop-for-generative-ai
64. Coconote. (n.d.). *Evaluator and human involvement*. Coconote. https://coconote.app/notes/0c65cec5-0763-416d-b71a-006037c13ee2/transcript
65. OpenAI. (n.d.). *Best practices for prompt engineering with the OpenAI API*. OpenAI. https://help.openai.com/en/articles/6654000-best-practices-for-prompt-engineering-with-the-openai-api
...
67. Microsoft. (n.d.). *Prompt engineering techniques*. Microsoft Azure. https://learn.microsoft.com/en-us/azure/ai-foundry/openai/concepts/prompt-engineering
68. Google Cloud. (n.d.). *What is prompt engineering?* Google Cloud. https://cloud.google.com/discover/what-is-prompt-engineering
69. Mirascope. (n.d.). *Prompt engineering best practices*. Mirascope. https://mirascope.com/blog/prompt-engineering-best-practices