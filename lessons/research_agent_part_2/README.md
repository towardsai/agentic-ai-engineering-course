...
TAVILY_API_KEY=your-tavily-api-key-here     # optional; required only for Tavily
...
Perplexity is selected by default. To use Tavily for every web-search call, set `WEB_SEARCH_PROVIDER=tavily`.
You can also override the configured provider for one call by passing
...
This loads the built‑in instructions that describe the full multi‑step workflow (URL extraction, scraping, Perplexity-by-default research loop with optional Tavily support, source selection, final research file creation, etc.).
...
3. Run multiple rounds of Perplexity-powered research (or Tavily when configured)
...
- **“API key missing” or HTTP 401/403 errors**
  - Recheck your `.env` files and/or exported environment variables for `GOOGLE_API_KEY`, `PPLX_API_KEY`, and `FIRECRAWL_API_KEY`. If using Tavily, also check `TAVILY_API_KEY`.
