...
3. Repeat the following research loop for 3 rounds:
    ...
    3.2. Run the "run_web_research" tool with the article guideline ID and the new queries. It uses Perplexity by
    default and appends normalized results to the legacy perplexity_results field in the database. If the user
    explicitly requests Tavily, pass provider="tavily" instead. The original "run_perplexity_research" tool remains
