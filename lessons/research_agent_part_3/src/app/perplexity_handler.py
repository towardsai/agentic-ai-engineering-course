...
def compute_next_source_id_from_text(existing_results: str | None) -> int:
    """
    Compute the next available source ID from existing perplexity results text.

    Args:
        existing_results: Existing perplexity results markdown text (may be None/empty)

    Returns:
        Next available source ID (starting from 1 if no existing results)
    """
    if not existing_results:
        return 1
    pattern = re.compile(r"### Source \[(\d+)\]:")
    max_id = 0
    for line in existing_results.splitlines():
        match = pattern.match(line)
        if match:
            max_id = max(max_id, int(match.group(1)))
    return max_id + 1
...
def format_perplexity_results(
    ...
    """
    Format perplexity results as markdown string.

    Args:
        query: The search query
        answer_by_source: Dict mapping local source IDs to answers
        citations: Dict mapping local source IDs to URLs
        starting_id: Starting global source ID

    Returns:
        Tuple of (formatted markdown string, next available source ID)
    """
    lines = []
    ...
    for local_id, url in citations.items():
        ...
        if not content:
            continue
        lines.append(f"### Source [{current_id}]: {url}\n")
