...
This lesson will provide you with a framework to make this important decision. We will explain the two core methodologies, explore use cases where each approach is most effective, and analyze state-of-the-art examples from leading AI companies. By the end, you will understand the fundamental trade-offs and be equipped to design systems that use the best of both worlds.

## Understanding the Spectrum: From Workflows to Agents

To begin, you will take a brief look at what LLM workflows and AI agents are. At this point, the focus is not on the technical specifics but on their core properties and how they are used. This foundational understanding will help you recognize the patterns and trade-offs that shape modern AI systems.

An **LLM workflow** is a sequence of tasks, including LLM calls and other operations like reading from a database, that is largely predefined and orchestrated by developer-written code. The steps are defined in advance, resulting in predictable, rule-based execution paths. Think of it as a factory assembly line, where each station performs a specific, repeatable action. The entire process is under the developer's control, making it transparent and testable. In future lessons, we will explore common workflow patterns like chaining, routing, and orchestrator-worker in detail.
...
Both workflows and agents require an orchestration layer, but its nature differs significantly. In a workflow, the orchestrator is like a conductor following a musical score, executing a defined plan with precision. It ensures that each step happens in the correct order and that data flows smoothly from one component to the next. In an agent, the orchestrator is more like a coach for a sports team, providing the overall goal and available plays (tools) but empowering the LLM (the player) to make real-time decisions on the field. It facilitates the LLM's dynamic planning and execution, adapting to the unfolding situation. This distinction between developer-defined logic and LLM-driven autonomy is the fundamental difference between the two approaches.
...
However, workflows can be rigid. They require more upfront development time to engineer each step and can struggle with unexpected scenarios that fall outside their predefined logic. This rigidity means that adding new features can become complex as the application grows, similar to the challenges of maintaining traditional software [[13]](https://www.deloitte.com/us/en/insights/industry/technology/technology-media-and-telecom-predictions/2025/autonomous-generative-ai-agents-still-under-development.html).

**AI agents** excel at open-ended, dynamic problems where the solution path is unclear. This includes tasks like complex research, dynamic problem-solving like debugging code, or interactive task completion in unfamiliar environments. Their strength lies in adaptability and flexibility. The trade-off is a loss of reliability. Agents are non-deterministic, so their performance, cost, and latency can vary with each run.

They are also harder to debug and often require larger, more expensive LLMs to reason effectively. There are even stories of early coding agents deleting entire codebases, prompting jokes like, "Anyway, I wanted to start a new project." This unpredictability makes them less suitable for high-stakes environments where errors have significant consequences [[9]](https://ardor.cloud/blog/common-ai-agent-deployment-issues-and-solutions), [[11]](https://www.strata.io/blog/agentic-identity/hidden-identity-challenges-ai-agents-hybrid-environment-1a/).
...
To build your intuition, you will now see the most common patterns used to build AI systems. We will explore these in depth in future lessons, but for now, we will keep the explanations high-level.

For **LLM workflows**, three patterns are fundamental:
*   **Chaining and routing** automate sequences of LLM calls. Chaining links steps together, where the output of one becomes the input for the next. This is useful for multi-stage tasks like summarizing a document and then translating the summary. Routing acts as a conditional gate, directing the workflow down different paths based on the input, like sending a customer query to either the "billing" or "technical support" department. This helps in breaking down complex tasks into manageable, specialized steps [[26]](https://www.revanthquicklearn.com/post/understanding-workflow-design-patterns-in-ai-systems), [[27]](https://www.philschmid.de/agentic-pattern), [[29]](https://docs.aws.amazon.com/prescriptive-guidance/latest/agentic-ai-patterns/workflow-for-routing.html).
...
*   The **orchestrator-worker** pattern involves a central orchestrator LLM that dynamically breaks down a task, delegates sub-tasks to specialized worker LLMs, and synthesizes the final result. For example, when asked to write a market analysis report, the orchestrator might assign one worker to research competitors, another to analyze financial data, and a third to write the introduction, before combining their outputs. This provides a bridge between structured workflows and dynamic agentic behavior [[16]](https://www.anthropic.com/research/building-effective-agents).
...
*   The **evaluator-optimizer** pattern creates a feedback loop for self-correction. A generator LLM produces an output, an evaluator LLM assesses it against certain criteria (like factual accuracy or tone), and the generator refines the output based on the feedback. This repeats until a quality threshold is met, much like a human writer revising a draft with an editor's comments [[14]](https://javaaidev.com/docs/agentic-patterns/patterns/evaluator-optimizer/), [[16]](https://www.anthropic.com/research/building-effective-agents).
...
For **AI agents**, the most prominent pattern is **ReAct (Reason and Act)**. This is the core operational loop for most modern agents. The agent autonomously decides what action to take, interprets the output of that action, and repeats the cycle until the task is complete. This requires several core components: an LLM for reasoning, a set of tools for taking actions (e.g., searching the web, running code), and memory to maintain context (both short-term for the current task and long-term for past experiences). We will dedicate several future lessons to explaining ReAct, tools, and memory in detail.
...
To make these concepts tangible, you will now see three examples, progressing from a simple workflow to a complex hybrid system.
...
Writing code can be a slow process, involving reading documentation, understanding new codebases, and learning new languages. A coding assistant can significantly speed this up. Based on our latest research from August 2025, Gemini CLI is an open-source tool implemented in TypeScript that uses a single-agent architecture based on the ReAct pattern to assist developers. Similar tools in this space include Cursor, Windsurf, Claude Code, and Warp [[3]](https://cloud.google.com/gemini/docs/codeassist/gemini-cli), [[4]](https://blog.google/technology/developers/introducing-gemini-cli-open-source-ai-agent/).
...
1.  **Context Gathering**: The agent loads the project's directory structure, available tools, and conversation history into its working memory.
...
4.  **Tool Execution**: Once approved, the agent executes its planned actions using a variety of tools. These can include file system access to read code with `grep`, coding tools for interpreting and generating diffs, web search for accessing documentation, and version control integration with `git` to commit changes.
...
*   **Reliability Issues:** Your agent works perfectly in demos but becomes unpredictable with real users. LLM reasoning failures can compound through multi-step processes, leading to unexpected and costly outcomes. For example, an agent might get stuck in a loop, repeatedly calling the same tool without making progress, or hallucinate incorrect information when it lacks sufficient context. Only 30% of generative AI pilots ever reach full production, largely due to a lack of trust in their output [[13]](https://www.deloitte.com/us/en/insights/industry/technology/technology-media-and-telecom-predictions/2025/autonomous-generative-ai-agents-still-under-development.html).
*   **Context Limits:** Systems struggle to maintain coherence across long conversations, gradually losing track of their purpose. This "context drift" can cause an agent to forget initial instructions or user preferences, leading to irrelevant or incorrect actions. Ensuring consistent output quality across different agent specializations presents a continuous challenge.
*   **Data Integration:** Building pipelines to pull information from Slack, web APIs, SQL databases, and data lakes while ensuring only high-quality data is passed to your AI system is a constant struggle, following the "garbage-in, garbage-out" principle. Enterprises report significant difficulties connecting diverse systems, with 42% of organizations using eight or more data sources [[9]](https://ardor.cloud/blog/common-ai-agent-deployment-issues-and-solutions).
*   **Cost-Performance Trap:** Sophisticated agents deliver impressive results but can cost a fortune per user interaction, making them economically unfeasible for many applications. A single complex query could trigger dozens of expensive LLM calls, and operational costs can increase by 30% annually as workloads rise [[9]](https://ardor.cloud/blog/common-ai-agent-deployment-issues-and-solutions).
*   **Security Concerns:** Autonomous agents with powerful write permissions could send wrong emails, delete critical files, or expose sensitive data if not properly sandboxed and monitored. Many agents run without proper identity controls, leading to insecure workarounds like hardcoded secrets that create significant vulnerabilities [[11]](https://www.strata.io/blog/agentic-identity/hidden-identity-challenges-ai-agents-hybrid-environment-1a/).

These challenges are solvable. In the next lesson, we will cover structured outputs to improve reliability. We will also explore workflow patterns, tools, planning, and memory in detail throughout this course. By the end, you will have the knowledge to build AI systems that are reliable, cost-effective, and secure. You will know when to use workflows versus agents and how to build effective hybrid systems that work in the real world.

## References

1. (n.d.). *A developer’s guide to building scalable AI: Workflows vs agents*. Towards Data Science. https://towardsdatascience.com/a-developers-guide-to-building-scalable-ai-workflows-vs-agents/
...
8. Karpathy, A. (n.d.). *Software in the era of AI* [Video]. YouTube. https://youtu.be/LCEmiRjPEtQ
...
10. Liu, J. (n.d.). *Building production-ready RAG applications* [Video]. YouTube. https://www.youtube.com/watch?v=TRjq7t2Ms5I
...
20. (n.d.). *Real agents vs. workflows: The truth behind AI 'agents'* [Video]. YouTube. https://www.youtube.com/watch?v=kQxr-uOxw2o&t=1s
...
37. (n.d.). *Andrej Karpathy: Software in the era of AI* [Video]. YouTube. https://www.youtube.com/watch?v=LCEmiRjPEtQ
