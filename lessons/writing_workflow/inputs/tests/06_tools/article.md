...
In our previous lessons, we built a solid foundation for creating AI applications. We learned about workflows, context engineering, structured outputs, and the core patterns for orchestrating tasks. But so far, our systems can only process information. They cannot *act*. An LLM is a brain in a jar. It can think, but it has no hands to interact with the world. This is the lesson where we give it hands.
...
1.  First, we set up our Gemini client and define some constants. The `DOCUMENT` constant will be used to mock the content of a PDF file.
    ...
    # Define the model ID and a mock document for our examples
...
2.  Next, we define the mock Python functions that will serve as our tools. To keep the code simple, these functions return hardcoded data instead of interacting with real APIs. The first tool simulates searching for a file on Google Drive.
    ```python
    def search_google_drive(query: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: A dictionary representing the search results.
    
        Example:
            >>> search_google_drive(query="Q3 earnings report")
...
3.  The second tool mocks sending a message to a Discord channel.
    ```python
    def send_discord_message(channel_id: str, message: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: A dictionary confirming the action.
    
        Example:
            >>> send_discord_message(channel_id="#finance", message="Here is the summary...")
            {'status': 'success', 'status_code': 200, 'channel': '#finance', 'message_preview': 'Here is the summary...'}
        """
...
4.  Our final tool provides a summary of a financial report.
    ```python
    def summarize_financial_report(text: str) -> str:
        ...
        Example:
            >>> summarize_financial_report(text="...")
            "The Q3 2023 earnings report shows strong performance..."
        """
        # This function returns a fixed summary for demonstration purposes.
...
7.  Here is the content of the `tools_schema` variable.
    ```python
    tools_schema
    ```
    It outputs:
    ```text
    [{'name': 'search_google_drive_example', 'description': 'Search for files in Google Drive.', 'parameters': {'type': 'object', 'properties': {'query': {'type': 'string', 'description': 'The query parameter'}}, 'required': ['query']}}, {'name': 'send_discord_message_example', 'description': 'Send a message to a Discord channel.', 'parameters': {'type': 'object', 'properties': {'channel_id': {'type': 'string', 'description': 'The channel_id parameter'}, 'message': {'type': 'string', 'description': 'The message parameter'}}, 'required': ['channel_id', 'message']}}, {'name': 'summarize_financial_report_example', 'description': 'Summarize the contents of a financial report.', 'parameters': {'type': 'object', 'properties': {'text': {'type': 'string', 'description': 'The text parameter'}}, 'required': ['text']}}]
    ```

8.  Let's test it. We use the same user prompt as before.
    ```python
    USER_PROMPT = """
    Please find the Q3 earnings report on Google Drive and send a summary of it to 
    the #finance channel on Discord.
...
9.  We execute the tool call using our `call_tool` function.
    ```python
    call_tool(response.text, tools_by_name=tools_by_name)
    ```
    It outputs:
...
6.  The `function_call` object contains the `name` of the tool and the `args` as a dictionary.
    ```python
    function_call.args
    ```
    It outputs:
...
7.  We can access the handler from our `TOOLS_BY_NAME` registry and execute it.
    ```python
...
8.  We can simplify our `call_tool` function to work directly with Gemini's `FunctionCall` object.
    ```python
...
9.  Now, calling the tool is a single, clean step.
    ```python
...
5.  Finally, we validate the arguments by instantiating our `DocumentMetadata` model with them.
    ...
    ```
The `document_metadata` object is now a validated, type-safe Pydantic object, ready for reliable use in our application. This pattern is essential for building robust AI agents that require structured data.
