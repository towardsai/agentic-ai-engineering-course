...
This is where tools, or function calling, come in. For an AI Engineer, tools are what transform an LLM from a simple text generator into an agent that can take action. Understanding how an agent works with tools is critical to building, improving, debugging, and monitoring them. We will open the black box and see exactly how the magic happens.
...
Before we start coding, let's clarify why LLMs need tools. At their core, LLMs are powerful pattern matchers and text generators. They are trained on vast datasets and excel at language-based tasks. However, they have a fundamental limitation: they cannot perform actions or access information outside of their training data on their own. Their knowledge is static and they have no direct connection to the external world.

Tools bridge this gap. You can think of the LLM as the brain, while tools are its "hands and senses." They allow the LLM to perceive and act in the world beyond its textual interface, transforming it from a passive text generator into an active AI agent.
...
*   Executing code in languages like Python or JavaScript to perform precise calculations or data manipulation [[5]](https://aman.ai/primers/ai/agents/).
...
We will learn how a tool is defined, what its schema looks like, how the LLM discovers available tools, how to call them, and how to interpret their output.
...
3.  For the LLM to understand these tools, we must define a schema for each one. The schema, typically in JSON format, describes the tool's name, its purpose, and the parameters it accepts.
    ...
    ```
    <aside>
    💡 This schema format is the industry standard for modern LLM providers like OpenAI and Google [[6]](https://platform.openai.com/docs/guides/function-calling), [[7]](https://ai.google.dev/gemini-api/docs/function-calling).
    </aside>
...
8.  In practice, the LLM uses the `description` field from the tool schema to decide if a tool is appropriate for a user's query. This is why clear and distinct tool descriptions are critical for building successful agents. Vague descriptions like "Tool used to search documents" and "Tool used to search files" can confuse the model. Instead, be explicit: "Tool used to search documents on Google Drive" and "Tool used to search files on the local disk."

    Clear tool descriptions, combined with explicit user prompts, ensure the agent can match intent to the correct tool. This becomes vital when scaling to dozens of tools per agent. Once a tool is selected, the model generates the function name and arguments as a structured output, like JSON. This capability is not accidental; modern LLMs are specifically instruction-fine-tuned to interpret tool schemas and produce valid tool calls [[8]](https://python.langchain.com/docs/concepts/tools/).
...
5.  The decorated function is now a `ToolFunction` object.
    It has a `schema` attribute that was automatically generated, which is identical to the one we created manually. It also contains the original function handler in the `func` attribute.

6.  Let's test it. We use the same user prompt as before.
    ```python
    USER_PROMPT = """
    Please find the Q3 earnings report on Google Drive and send a summary of it to 
    the #finance channel on Discord.
...
7.  We execute the tool call using our `call_tool` function.
    ```python
    call_tool(response.text, tools_by_name=tools_by_name)
    ```
    It outputs:
...
By using Gemini's native SDK, we reduced dozens of lines of manual setup to just a few. This approach is not unique to Google; other popular APIs like OpenAI and Anthropic use a similar logic, so the concepts you have learned here are broadly applicable [[6]](https://platform.openai.com/docs/guides/function-calling).

## Using Pydantic models as tools for on-demand structured outputs

In Lesson 4, we learned how to generate structured outputs. We can combine that knowledge with tool calling to create a powerful pattern: using a Pydantic model as a tool. This is useful in agentic workflows where you perform several intermediate steps that produce unstructured text (which is easy for an LLM to interpret) and then dynamically decide when to output the final answer in a structured format for downstream processing [[10]](https://xebia.com/blog/enforce-and-validate-llm-output-with-pydantic/), [[11]](https://pydantic.dev/articles/llm-intro).
...
While powerful, this simple loop has significant limitations [[12]](https://www.promptingguide.ai/techniques/react), [[13]](https://aman.ai/primers/ai/agents/):
...
<aside>
💡 A quick optimization is to run independent tools in parallel. If a prompt requires fetching financial news and stock prices simultaneously, and these tasks don't depend on each other, calling them in parallel can significantly reduce latency [[14]](https://ai.google.dev/gemini-api/docs/function-calling).
</aside>
...
1. (n.d.). *Primers*. Aman.ai. https://aman.ai/primers/ai/agents/
2. (n.d.). *Comprehensive Guide to Integrating Tools and APIs with Language Models*. Mercity.ai. https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models
3. (n.d.). *Deep Research (DR) agents*. arXiv. https://arxiv.org/html/2506.18096v1
4. (n.d.). *Retrieval-Augmented Generation: Everything You Need to Know About RAG in AI*. Weka. https://www.weka.io/learn/guide/ai-ml/retrieval-augmented-generation/
5. (n.d.). *Primers*. Aman.ai. https://aman.ai/primers/ai/agents/
6. (n.d.). *Function calling*. OpenAI. https://platform.openai.com/docs/guides/function-calling
7. (n.d.). *Function calling with the Gemini API*. Google AI for Developers. https://ai.google.dev/gemini-api/docs/function-calling
8. (n.d.). *Tools*. LangChain. https://python.langchain.com/docs/concepts/tools/
9. (n.d.). *Tools*. LangChain. https://python.langchain.com/docs/concepts/tools/
10. (n.d.). *Enforce and validate LLM output with Pydantic*. Xebia. https://xebia.com/blog/enforce-and-validate-llm-output-with-pydantic/
11. (n.d.). *Validating LLM Output with Pydantic*. Pydantic. https://pydantic.dev/articles/llm-intro
12. Yao, S., Zhao, J., Yu, D., Du, N., Shafran, I., Narasimhan, K., & Cao, Y. (2022). *ReAct: Synergizing Reasoning and Acting in Language Models*. arXiv. https://www.promptingguide.ai/techniques/react
13. (n.d.). *Primers*. Aman.ai. https://aman.ai/primers/ai/agents/
14. (n.d.). *Function calling with the Gemini API*. Google AI for Developers. https://ai.google.dev/gemini-api/docs/function-calling
15. (n.d.). *Retrieval-Augmented Generation: Everything You Need to Know About RAG in AI*. Weka. https://www.weka.io/learn/guide/ai-ml/retrieval-augmented-generation/
16. (n.d.). *LLM Integration*. Mirascope. https://mirascope.com/blog/llm-integration
17. (n.d.). *Microsoft brings Copilot AI-powered web search mode on Bing: How it works*. Business Standard. https://www.business-standard.com/technology/tech-news/microsoft-brings-copilot-ai-powered-web-search-mode-on-bing-how-it-works-125022500477_1.html
18. (n.d.). *Setting up a secure Python sandbox for LLM agents*. Dida.do. https://dida.do/blog/setting-up-a-secure-python-sandbox-for-llm-agents
19. (n.d.). *Secure code execution*. Hugging Face. https://huggingface.co/docs/smolagents/en/tutorials/secure_code_execution
20. (n.d.). *Comprehensive Guide to Integrating Tools and APIs with Language Models*. Mercity.ai. https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models
21. (n.d.). *What is Tool Calling? Connecting LLMs to Your Data*. YouTube. https://www.youtube.com/watch?v=h8gMhXYAv1k
22. (n.d.). *ReAct vs Plan-and-Execute: A Practical Comparison of LLM Agent Patterns*. dev.to. https://dev.to/jamesli/react-vs-plan-and-execute-a-practical-comparison-of-llm-agent-patterns-4gh9
23. (n.d.). *Agentic Design Patterns Part 3, Tool Use*. DeepLearning.AI. https://www.deeplearning.ai/the-batch/agentic-design-patterns-part-3-tool-use/
24. Gao, S., Dwivedi-Yu, J., Yu, P., Tan, X. E., Pasunuru, R., Golovneva, O., Sinha, K., Celikyilmaz, A., Bosselut, A., & Wang, T. (n.d.). *Efficient Tool Use with Chain-of-Abstraction Reasoning*. arXiv. https://arxiv.org/pdf/2401.17464v3
25. (n.d.). *Building AI Agents from scratch - Part 1: Tool use*. Swirl AI. https://www.newsletter.swirlai.com/p/building-ai-agents-from-scratch-part
26. (n.d.). *Tool Calling Agent From Scratch*. YouTube. https://www.youtube.com/watch?v=ApoDzZP8_ck
27. (n.d.). *Function calling with OpenAI's API*. OpenAI. https://platform.openai.com/docs/guides/function-calling
28. (n.d.). *Function calling with the Gemini API*. Google AI for Developers. https://ai.google.dev/gemini-api/docs/function-calling