...
2. Next, we import the required libraries. We rely on `google.genai` for interacting with the model. We also import `pydantic` and `enum`. These are crucial for defining the structured data our agent will use. `pydantic` allows us to create strict data models for our agent's messages and tool calls, ensuring that the data flowing through our control loop is valid and typed. `enum` helps us define fixed roles for our messages (like "User" or "Thought"), preventing typo-related bugs in our state management.
    ```python
...
3. Let's inspect what this prompt actually looks like when rendered. This is crucial for debugging—if the LLM doesn't know what tools it has, it can't reason about them.
    ```python
...
Gemini's SDK provides a powerful feature where it can automatically inspect Python functions and convert them into the JSON schema required by the model.
...
The magic of ReAct happens when the agent *observes* the result of its action. In our loop, this is handled explicitly. When `generate_action` returns a `ToolCallRequest`, we do not just print it; we execute it.
