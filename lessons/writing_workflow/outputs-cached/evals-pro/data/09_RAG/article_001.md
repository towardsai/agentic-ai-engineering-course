...
This step often involves managing the "context window" of the LLM. Since models have a limit on how much text they can process, augmentation involves selecting only the most pertinent parts of the retrieved documents, potentially summarizing them, and arranging them in a way that maximizes the model's attention on the key facts.
...
**Generate:** You construct a prompt containing the user's query and the text from those top-k chunks. You send this to the LLM, which generates the final answer. As we learned in **Lesson 4**, using structured outputs here is crucial if you need citations or specific formats. This ensures the model doesn't just chat, but provides a rigorous answer grounded in the provided text [[3]](https://decodingml.substack.com/p/rag-fundamentals-first?utm_source=publication-search).
...
A re-ranker is a specialized model (like a Cross-Encoder) that takes the user's query and a candidate document and outputs a precise relevance score. Unlike the bi-encoder models used for fast vector search (which process the query and document separately), a cross-encoder processes them together, allowing it to understand the deep interaction between the two texts.

Because this process is computationally expensive, we don't run it on the whole database. Instead, we retrieve the top 50 documents using fast vector search, and then use the re-ranker to sort them and keep only the top 5. This substantially improves the quality of the context passed to the LLM, ensuring that the limited context window is filled with only the most pertinent information [[4]](https://decodingml.substack.com/p/your-rag-is-wrong-heres-how-to-fix?utm_source=publication-search).
...
*   **Semantic Chunking:** Splits text based on meaning rather than character count. It uses an embedding model to detect when the topic of the text changes and places the split there, keeping coherent thoughts together.
...
Standard RAG struggles with questions like, "Which incidents were caused by weekend deploys that also touched the login service?" This requires hopping between data points: Change Record → Deploy Time → Affected Service → Incident Ticket. GraphRAG excels here by traversing these relationships to retrieve the full context, solving the "multi-hop" reasoning problem that vector search often fails at. It builds a structured map of your data, allowing the system to "walk" from one entity to another to find the answer [[5]](https://arxiv.org/html/2404.16130).
