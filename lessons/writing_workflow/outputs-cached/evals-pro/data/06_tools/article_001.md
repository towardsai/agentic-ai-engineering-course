...
The best way to understand how LLMs use tools is to build the mechanism yourself. While modern Software Development Kits (SDKs) abstract this away, knowing what happens under the hood is critical for debugging when your agent inevitably tries to call a tool that doesn't exist or hallucinates arguments.
...
7.  To execute this, we need to parse the string output. We extract the JSON, convert it to a dictionary, look up the function in our `TOOLS_BY_NAME` registry, and call it. This step is where the "magic" happens—converting the intent into action.
    ```python
...
8.  Finally, we close the loop. We take this raw data and send it back to the LLM so it can interpret the result for the user. This step ensures the user receives a natural language answer rather than a raw JSON dump.
    ```python
    response = client.models.generate_content(
        model=MODEL_ID,
...
*   **Loops:** It's easy for an agent to get stuck calling `search_google_drive` endlessly if it doesn't find the file. Imagine an agent that keeps searching for "Q3 Report," failing, and trying again with the exact same query, burning through your API credits in seconds.
