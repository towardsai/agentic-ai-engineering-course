...
To truly master context engineering, we need to dissect the anatomy of a prompt. It is not just a string of text; it is a carefully layered architecture of information. Let's walk through the lifecycle of a single interaction to see how context is built, used, and stored.

The workflow follows a specific chain of events: **User Input -> Long-term Memory -> Short-Term Working Memory -> Context -> Prompt Template -> Prompt -> LLM Call -> Answer -> Short-Term Working Memory -> Long-Term Memory -> Repeat**.

It all begins with the **User Input**. This is the trigger. It might be a question like "What's on my calendar today?" or a command like "Refactor this code." This input is the seed that determines what other information we need to fetch.

Once we have the input, we query our **Long-term Memory**. We do not fetch everything. We query our Episodic Memory to see if we know anything about the user's preferences (e.g., "User prefers meetings in the afternoon"). We query our Semantic Memory to get factual data (e.g., retrieving calendar events from a database). We also load our Procedural Memory, which defines the tools available (e.g., a `calendar_tool`) and the system instructions.

Next, we look at the **Short-Term Working Memory**. This contains the conversation history so far. If the user previously said, "I'm feeling tired today," that is stored here. We combine the new user input, the retrieved long-term data, and the relevant short-term history into a unified state.

This aggregated information forms the **Context**. But raw data is messy. We cannot just throw JSON objects and database rows at the model. We need to format it. This is where the **Prompt Template** comes in. The template is a blueprint. It has slots for `{{user_input}}`, `{{history}}`, and `{{retrieved_docs}}`. We inject our context into this template to create the final **Prompt**.

We send this Prompt to the **LLM Call**. The model processes the instructions and data. It generates an **Answer**. This answer might be a final text response to the user, or it might be a tool call (e.g., "Move meeting to 3 PM").

Crucially, the process does not end here. The Answer is fed back into the **Short-Term Working Memory** so the agent remembers what it just said. If the interaction revealed new permanent information (e.g., the user says "I moved to New York"), we update the **Long-term Memory**.

Finally, the cycle **Repeats**. The system waits for the next user input, ready to rebuild the context all over again with the updated state.

Now, let's look closer at the specific components that populate this lifecycle.

### Short-term Working Memory: The Agent's RAM

Short-term memory is the state of the agent. It is transient. It exists only for the duration of the session.

*   **User Input:** The raw text or data provided by the user in the current turn.
*   **Message History:** A chronological log of the conversation. This allows the model to resolve references like "it" or "that" by looking back at previous turns.
*   **Internal Thoughts:** In advanced agents (like ReAct agents, which we will cover in Lesson 7), the model generates internal reasoning traces. It might think, "The user asked for weather. I should check the location first." These thoughts are part of the working memory, helping the model stay on track.
*   **Tool Calls and Outputs:** If the agent executes a Python script or queries an API, the code it wrote and the result it got back (e.g., "Error: File not found" or "200 OK") are stored here. This feedback loop is essential for error correction.

### Long-term Memory: The Agent's Hard Drive

Long-term memory is persistent. It survives after the session ends.

*   **Procedural Memory (The "How"):** This is the code and configuration that defines the agent's behavior.
    *   *System Prompt:* The core personality and rules (e.g., "You are a helpful coding assistant. Always write tests.").
    *   *Tools:* The definitions of functions the agent can call (e.g., `search_web(query: str)`).
    *   *Structured Output Schemas:* The JSON or Pydantic models that define how the agent should format its response.
*   **Episodic Memory (The "Who"):** This is the autobiographical memory of the system. It stores specific details about the user and past interactions.
    *   *User Preferences:* "The user prefers dark mode." "The user is a beginner in Python."
    *   *Past Experiences:* "Last week, we debugged a React app together."
    *   This is often implemented using vector databases to perform semantic searches over past conversations.
*   **Semantic Memory (The "What"):** This is the factual knowledge base.
    *   *Internal Knowledge:* Company documents, wikis, and databases.
    *   *External Knowledge:* Information retrieved from the open internet via search APIs or web scrapers.

These components are not static. They are dynamic. A major part of context engineering is writing the logic that decides *which* of these millions of data points to pull into the limited context window for any given prompt.
...
The primary challenge is the **context window limit**. Every model has a hard limit on how much information it can process at once. The context window is the RAM on your laptop. If you have 32GB of RAM, you cannot load a 100GB dataset into memory. You have to be selective. Even with models boasting 1M+ token windows, latency and cost scale linearly with input size. A massive context window is technically possible but often economically unviable for real-time applications.

This leads to **information overload**, often called the "lost-in-the-middle" problem. LLMs are notorious for paying attention to the beginning and end of a prompt while ignoring the middle [[2]](https://arxiv.org/pdf/2507.13334). If you stuff 50 documents into the context, the model will likely miss the important detail hidden in document 25. Processing the middle is a lottery. This phenomenon forces us to be extremely precise about the order and quantity of information we present.

Then we have **context drift**. This occurs when your memory contains conflicting views of the truth. Over time, a user might change their mind. Data sources might update. You might have one memory saying "The project deadline is Friday" and a newer one saying "The deadline is Monday." This confuses the LLM. It does not know which fact is current. Without a mechanism to rank information by recency or reliability, the model might hallucinate a hybrid reality that is incorrect.

Finally, there is **tool confusion**. This happens in two scenarios. First, if you give an agent too many actions to choose from—usually over 100—it gets paralyzed. It cannot decide which tool is best. It is like opening a toolbox and finding 50 different screwdrivers that all look slightly different.

Second, if the descriptions of your actions are unclear or overlap, the model will fail. If you have a "Search Web" tool and a "Find Info" tool with similar descriptions, the model will flip a coin. That is not engineering. That is gambling. You must write crisp, distinct documentation for every tool you expose to the agent.

## Key strategies for context optimization

Initially, AI apps were simple chatbots over a single document. Today, we build systems that access multiple knowledge bases and execute complex actions. We must manage this complexity while keeping costs down and performance up.
...
Apply **Temporal Relevance**. Rank data by time and cut off old, irrelevant points. If you are analyzing stock prices, data from 2020 is likely noise. Prioritize the most recent information to ensure the model acts on the current state of the world. You can implement decay functions where older memories have lower retrieval scores unless they are explicitly marked as "evergreen" facts.
...
One method is to use an LLM to **summarize past interactions**. You replace ten messages with a single paragraph summary. This retains the core meaning but drastically reduces the token count. It is a lossy process, but it is often a necessary trade-off to keep long-running sessions viable. You can trigger this summarization automatically when the history exceeds a certain token threshold (e.g., 4000 tokens).
...
**Deduplication** is another simple but effective tactic. Often, conversation histories contain repeated information or redundant system acknowledgments (e.g., "Okay, I will do that"). Removing these redundant turns can save 10-20% of your tokens without affecting the model's understanding.
...
This isolation also helps with security and privacy. You can ensure that sensitive data (like PII) is only passed to the specific worker that needs it, rather than being exposed to the entire system context.
...
Also, consider using **YAML** instead of JSON when inputting structured data. YAML is often more token-efficient. It uses fewer brackets and quotes. It is also easier for models to read, saving you cost and window space. For large datasets, this small optimization can compound into significant savings.
...
To build this stack, we recommend using **Gemini** for its multimodal reasoning capabilities and low cost. Its large context window allows for flexibility, but as we learned, we should still optimize what we put in it.

Use **LangGraph** as the orchestrator and memory virtual layer. LangGraph is excellent because it treats the agent's state as a first-class citizen, making it easy to manage the "Short-term Working Memory" we discussed. It allows you to define explicit graphs of execution, giving you control over the flow of context.

For databases, keep it simple—**PostgreSQL** often works perfectly for both vector and relational data using the `pgvector` extension. You do not always need a specialized vector database. PostgreSQL simplifies your infrastructure by keeping your relational data (user profiles) and semantic data (embeddings) in one place.

Finally, use **Opik** or **LangSmith** for observability. These tools allow you to inspect the exact prompt sent to the LLM. You can see if your context compression is working or if your RAG pipeline is retrieving irrelevant documents. Without this visibility, you are flying blind.
