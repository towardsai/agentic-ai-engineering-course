...
This happens because LLMs are fundamentally stateless. Their "internal knowledge" is frozen at the moment of training. They cannot learn by updating their weights after deployment. This is a challenge known in the field as "continual learning." To solve this, we rely on **Context Engineering**, which we introduced in Lesson 3. However, managing context is not just about formatting prompts; it is about deciding what information to keep, how to store it, and when to retrieve it.
...
*   **Cons:** It has the highest engineering overhead. Constructing a graph from unstructured text is complex and costly. Graph traversals can also be slower than simple vector lookups, potentially adding latency to the user experience [[7]](https://neo4j.com/blog/genai/knowledge-graph-vs-vectordb-for-retrieval-augmented-generation/).
...
Today, with models like Gemini 1.5 Pro offering massive context windows at lower costs, this constraint has relaxed [[1]](https://www.dhiwise.com/post/the-gemini-context-window-and-its-role-in-ai-precision), [[8]](https://cloud.google.com/vertex-ai/generative-ai/docs/models/gemini/2-5-pro). The new best practice is to preserve **raw detail**. Summarization is lossy; it strips away the emotional subtext and subtle hesitations that make an agent feel "human." While a summarized fact might say "User likes dogs," the raw log reveals "User mentioned that walking their dog George is the only thing keeping them sane this week." That nuance is gold for a personalized agent. Use summarization for indexing and search, but treat the raw log as your source of truth.

### Designing for the Product

Avoid the trap of over-engineering. Just because you *can* build a complex system with Semantic, Episodic, and Procedural memory doesn't mean you *should*. Start from your product's core needs [[9]](https://www.speakeasy.com/mcp/ai-agents/architecture-patterns).
...
In the next lesson, we will explore **Retrieval-Augmented Generation (RAG)** in detail, moving beyond simple memory lookups to building complex knowledge retrieval pipelines. We will also touch on how to handle **Multimodal** data, ensuring your agent can remember images and documents as well as text.
...
5. AI Agent Memory. (2024, July 22). GeeksforGeeks.  https://www.geeksforgeeks.org/artificial-intelligence/ai-agent-memory/ 

6. Vector database vs. graph database. (2023, October 24). Writer.  https://writer.com/engineering/vector-database-vs-graph-database/ 

7. Knowledge Graph vs. Vector Database for Retrieval-Augmented Generation. (2024, June 6). Neo4j.  https://neo4j.com/blog/genai/knowledge-graph-vs-vectordb-for-retrieval-augmented-generation/ 

8. Gemini 2.5 Pro. (n.d.). Google Cloud.  https://cloud.google.com/vertex-ai/generative-ai/docs/models/gemini/2-5-pro 

9. AI Agent Architecture Patterns. (n.d.). Speakeasy.  https://www.speakeasy.com/mcp/ai-agents/architecture-patterns 

10. Intelligent Memory Management for AI Agents. (2024, June 12). Savantly.  https://blog.savantly.net/intelligent-memory-management-for-ai-agents/ 
