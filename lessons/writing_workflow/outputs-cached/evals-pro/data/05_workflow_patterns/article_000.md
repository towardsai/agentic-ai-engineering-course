...
When I first started building AI applications, I fell into the "God Prompt" trap. I thought that if I just engineered the perfect, massive prompt, the LLM would magically handle everything: reasoning, formatting, fact-checking, and tone adjustment.

I was wrong. My system was brittle, impossible to debug, and hallucinated constantly.
...
We often see developers trying to stuff complex logic into a single LLM call. It seems efficient at first—one request, one response. But as your application grows, this approach becomes a nightmare.

When a prompt tries to do too much, you face several issues. First, debugging is nearly impossible. If the model fails, is it because it misunderstood the context, failed the formatting instruction, or hallucinated a fact? You don't know. Second, you run into "lost in the middle" phenomena, where models ignore instructions buried in long contexts [[1]](https://arxiv.org/abs/2307.03172). Finally, you lose modularity; you cannot optimize specific parts of the task without rewriting the entire prompt.
...
1.  We initialize our client and define our model.
    ```python
    import asyncio
...
2.  Next, we define our source material. In a real app, this would come from a vector database or an API, but for this example, we will mock it.
    ```python
    webpage_1 = {
        "title": "The Benefits of Solar Energy",
        "content": """
        Solar energy is a renewable powerhouse... (content regarding solar panels, costs, and savings)
        """,
    }
    ...
    webpage_2 = {
        ...
        "content": """
        Wind turbines are towering structures... (content regarding wind energy and intermittency)
        """,
    }
    ...
    webpage_3 = {
        ...
        "content": """
        Effective energy storage is the key... (content regarding batteries and grid stability)
        """,
    }
...
3.  Now, we attempt to do everything in one go. We ask for 10 FAQs, concise answers, and citations. We use Pydantic for structured output, which we learned about in the previous lesson.
    ```python
...
Think of it like a factory assembly line. You don't have one person build the entire car from scratch. You have one station for the chassis, one for the engine, and one for the wheels.

Chaining offers massive benefits for AI Engineering:
1.  **Modularity:** Each step focuses on one thing. You can tune the prompt for "Question Generation" without breaking the "Source Citation" logic.
2.  **Accuracy:** Simpler prompts are easier for LLMs to follow. The cognitive load on the model is lower [[3]](https://www.promptingguide.ai/techniques/prompt_chaining).
3.  **Debuggability:** If the answers are wrong, you check the Answer step. If the citations are wrong, you check the Source step. You know exactly where the failure occurred [[4]](https://www.vellum.ai/blog/what-is-prompt-chaining).
4.  **Model Selection:** You can use a cheap, fast model (like Gemini Flash) for simple tasks and a reasoning model (like Gemini Pro or GPT-4) for complex analysis [[5]](https://aws.amazon.com/blogs/machine-learning/multi-llm-routing-strategies-for-generative-ai-applications-on-aws/).

The downsides? It's slower (latency adds up) and potentially more expensive (more tokens). But for production systems, reliability usually trumps raw speed.
...
We create a specific function just for generating questions.

1.  We define a `QuestionList` schema and a focused prompt.
    ```python
...
Now we need a function that answers a single question. Notice we are not asking for citations here. We just want a good answer.

1.  We define a prompt that enforces using *only* the provided content.
    ```python
...
Finally, we ask the LLM to act as a librarian. Given the Q&A pair, which document did this come from?
...
1.  We loop through the questions generated in step 1, and for each one, we call step 2 and step 3.
    ```python
...
The result is robust and accurate. But wait—22 seconds for 4 questions? That is painfully slow. In a sequential chain, we are waiting for the LLM to finish thinking about Question 1 before it even looks at Question 2. This brings us to our next pattern.
...
This pattern is crucial for high-performance AI apps. It reduces the total latency from `sum(all_tasks)` to `max(slowest_task)`.

### Implementing Asyncio

We will use Python's `asyncio` to make concurrent calls to the Gemini API.

1.  We create async versions of our functions and a runner function that gathers the tasks.
    ```python
...
2.  Let's check the performance improvement.
    ```python
    start_time = time.monotonic()
...
We dropped from 22 seconds to ~9 seconds. That is a massive improvement for user experience.
...
1.  We define the `IntentEnum` and the classification function.
    ```python
...
Now we define the logic for each branch. In a real app, these might be complex RAG pipelines or database lookups. Here, we will use specialized prompts.
...
1.  We define prompts for Tech Support, Billing, and General questions.
    ...
    # ... (Similar prompts for Billing and General)
...
2.  Let's test it with a billing question.
    ```python
...
This architecture scales beautifully. If you need to add a "Sales" flow, you just add an Enum value and a new `elif` branch. You don't risk breaking the Technical Support logic.
...
This is different from simple parallelization because the *number* and *type* of tasks are determined at runtime by the LLM, not hardcoded by the developer.
...
The Orchestrator's job is to produce a plan. We use Pydantic to strictly define what a "Task" looks like.

1.  We define the `Task` schema and the Orchestrator function.
    ```python
    ...
    def orchestrator(query: str) -> list[Task]:
        # ... (Call LLM to get TaskList)
        return response.parsed.tasks
    ```

### Step 2: The Workers

Each worker is a function that takes specific parameters and returns a result. In a real system, these would call APIs. Here, we simulate the work.

1.  We implement the `handle_billing_worker`, `handle_return_worker`, and `handle_status_worker`.
    ```python
    def handle_billing_worker(invoice_number: str, original_user_query: str) -> BillingTask:
        # Extract specific concern using LLM, then simulate DB lookup
        # ...
        return BillingTask(...)
    
    def handle_return_worker(product_name: str, reason_for_return: str) -> ReturnTask:
        # Simulate generating RMA
        # ...
        return ReturnTask(...)
    ```
...
Once the workers finish, we have a pile of data. We need to turn that into a polite email.

1.  We define a `synthesizer` function that takes the list of results and formats them.
    ```python
...
2. (n.d.). *What are the common failure modes of complex, single-prompt LLM calls?*. arXiv. https://arxiv.org/pdf/2309.08181

3. (n.d.). *Prompt Chaining Guide*. Prompting Guide. https://www.promptingguide.ai/techniques/prompt_chaining

4. (n.d.). *What is Prompt Chaining?*. Vellum. https://www.vellum.ai/blog/what-is-prompt-chaining

5. (n.d.). *Multi-LLM routing strategies for generative AI applications on AWS*. Amazon Web Services. https://aws.amazon.com/blogs/machine-learning/multi-llm-routing-strategies-for-generative-ai-applications-on-aws/
