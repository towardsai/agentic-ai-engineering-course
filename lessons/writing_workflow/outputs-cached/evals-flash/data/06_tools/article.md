Here is the revised article, incorporating all the feedback and adhering to the specified profiles and guidelines:
...
We remember a project where our LLM-powered chatbot was brilliant at generating text, but it couldn't do anything practical. It could tell users about the weather, but it couldn't actually *fetch* the current temperature. It could explain how to send an email, but it couldn't *send* one. Our AI was stuck in a "read-only" mode, unable to interact with the real world.

In our journey through AI engineering, we have covered fundamental concepts. We started with the AI agent landscape, understanding its role and stack in Lesson 1. Next, we grasped the crucial difference between predefined LLM workflows and autonomous AI agents in Lesson 2. Then, we explored context engineering, the art of managing information flow to LLMs in Lesson 3. We also learned how to ensure reliable data extraction from LLM responses using structured outputs in Lesson 4. Finally, in Lesson 5, we implemented basic workflow ingredients like chaining, routing, parallel processing, and the orchestrator-worker patterns.

This lesson will explore agent tool usage. We will highlight that through tools, we can enable agents to take actions and interact with the external world. We will implement tool calling from scratch to understand how the LLM decides which tool to call, how it generates the correct parameters for the function, and how it calls the function. We will also explore using structured outputs as tools through Pydantic models. Next, we will examine scenarios requiring multiple tools and introduce tool chaining, before discussing the limitations that lead to more sophisticated patterns like ReAct. Finally, we will look at the most essential tool categories such as memory access and web search through RAG or code execution. All our ideas will be supported by code.

## Understanding Why Agents Need Tools

LLMs, at their core, are powerful pattern matchers and text generators. They predict the next most probable token based on their training data. This means they cannot, by themselves, perform actions in the real world or access information beyond their static knowledge cutoff. They live in a textual bubble, unable to interact with external systems or retrieve real-time data on their own. This is a fundamental limitation of standalone LLMs [[36]](https://www.projectpro.io/article/llm-limitations/1045).

Tools are what transform an LLM from a simple text generator into an agent that can take actions into the external world. The LLM acts as the brain, while tools serve as its "hands and senses." Tools are the bridge between the LLM's internal reasoning and the dynamic external world. With tools, an LLM becomes an AI agent that can perceive its environment, execute specific instructions, and perform actions [[38]](https://www.edpb.europa.eu/system/files/2025-04/ai-privacy-risks-and-mitigations-in-llms.pdf).

<img src="https://cdn.prod.website-files.com/640f56f76d313bbe39631bfd/650f6700a38686bf04abf9ae_oYM1xj0eyB4-37H6LC2k60wZN98clF_vgT6B7Rv4NTkmMqppzhWmw6fOeFw-Vp5m_daHM0Mas4j3DSf5dkVXQQTq_77WGU0GlopNEGhB5Cg-J26wDv6syQqO_1YnNIwaQw-hbOt2KF4tSukggX707J3p2npRfYjunpI.png" />
...
As seen in Image 1, the LLM processes user requests and, if an external action is needed, decides which tool to use. This decision is then passed to an integrator, which executes the tool via a tool execution pipeline. The result is parsed and fed back to the LLM, allowing it to formulate a coherent response or take further steps [[16]](https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models). This architecture enables LLMs to offload tasks, access current data, and perform actions that pure language modeling cannot achieve [[37]](https://arxiv.org/html/2504.14872v1).

Modern AI agents use various tools to extend their capabilities. These tools allow LLMs to access up-to-date resources and execute actions beyond text generation [[53]](https://www.projectpro.io/article/llm-limitations/1045). Some popular examples include:
*   **Accessing real-time information:** Tools that query APIs for current weather, news, or stock prices [[17]](https://aman.ai/primers/ai/agents/). This overcomes the static nature of pretrained models and provides dynamic, up-to-date context [[20]](https://www.weka.io/learn/guide/ai-ml/retrieval-augmented-generation/).
*   **Interacting with databases:** Tools that query structured databases (PostgreSQL) or data lakes (S3) to retrieve or analyze information [[18]](https://mirascope.com/blog/llm-integration). This is essential for enterprise applications that rely on proprietary data [[24]](https://www.k2view.com/what-is-retrieval-augmented-generation).
*   **Accessing long-term memory:** Tools that retrieve information from an agent's memory beyond its current context window. This capability is crucial for maintaining context over extended interactions and personalizing responses, a concept we will explore in Lesson 9 on Agent Memory & Knowledge [[21]](https://www1.hkexnews.hk/app/sehk/2025/107494/documents/sehk25062703133.pdf).
*   **Executing code:** A Python interpreter tool allows the agent to write and run code in a sandboxed environment for calculations, data manipulation, or visualizations [[19]](https://arxiv.org/html/2506.18096v1). This provides precise and customized solutions that standard functions cannot offer [[12]](https://dida.do/blog/setting-up-a-secure-python-sandbox-for-llm-agents).
*   **Performing precise calculations:** Tools that handle mathematical operations, sorting, filtering, or grouping data, going beyond the LLM's inherent computational limitations [[36]](https://www.projectpro.io/article/llm-limitations/1045). This ensures accuracy in tasks that require exact numerical results [[56]](https://arxiv.org/html/2412.04503v1).

These tools enable LLMs to overcome their static nature and perform dynamic, real-world tasks, transforming them into powerful AI agents [[47]](https://www.getdynamiq.ai/post/llm-agents-explained-complete-guide-in-2025).
...
This request-execute-respond flow is central to tool calling, as illustrated in Image 2. It allows the LLM to act as an orchestrator, determining function calls and arguments, while your code handles the actual execution and result communication [[49]](https://python.langchain.com/docs/how_to/function_calling).
...
1.  We define the imports, Gemini `client`, `MODEL_ID`, and `DOCUMENT` constants. `DOCUMENT` is used to mock PDF documents.
    ...
    """
    ```
    This initial setup provides the necessary environment and data for our tool-calling demonstration. The `DOCUMENT` constant simulates a financial report that our tools will interact with.

2.  We define three mock tools: `search_google_drive`, `send_discord_message`, and `summarize_financial_report`. These functions simulate real-world operations. All tools are mocked to keep the code simple and focus on the tools implementation.
    ```python
    ...
    ```
    These mock functions allow us to simulate interactions with external systems without needing actual API keys or complex setups. The docstrings and type hints are important, as they provide the metadata the LLM will use to understand each tool.

3.  We define the JSON schemas for the three tools. This schema is used as input to the LLM to understand what tool to use (based on the description) and how to call it (based on the parameters, which include variable names, types, descriptions, and whether they are required or optional). This schema is the industry standard when working with modern LLM providers such as OpenAI or Gemini [[71]](https://platform.openai.com/docs/guides/function-calling).
    ```python
    ...
    ```
    These JSON schemas provide a machine-readable contract for each tool. They precisely define the tool's purpose and how it expects to be invoked, which is essential for the LLM to generate correct function calls.

4.  We define the `TOOLS` registry, along with their `TOOLS_BY_NAME` and `TOOLS_SCHEMA` mappings. The `TOOLS` dictionary stores the handler function and its schema for each tool. `TOOLS_BY_NAME` provides a quick lookup for the function handler by name, and `TOOLS_SCHEMA` aggregates all schemas for passing to the LLM.
    ```python
    TOOLS = {
        "search_google_drive": {
    ...
    ```
    This setup creates a centralized registry for all available tools, making it easy to manage and provide them to the LLM.
...
5.  The `TOOLS_BY_NAME` mapping provides a dictionary that links tool names to their corresponding Python functions:
    ...
    ---------------------------------------------------------------------------
    ```
    This output confirms that our tool names are correctly mapped to their Python function handlers.
...
6.  Here is how the first element from the `TOOLS_SCHEMA` looks. It provides a structured definition for the `search_google_drive` tool:
    ...
    ```
    This JSON output shows the detailed schema for one of our tools, including its name, description, and expected parameters. This is the exact format the LLM will receive to understand the tool's capabilities.
...
7.  We define and explain the `TOOL_CALLING_SYSTEM_PROMPT`. This prompt guides the LLM on how and when to use the tools. It includes tool usage guidelines, the expected tool call format, response behavior, and a list of available tools enclosed by XML tags.
    ...
    **Critical formatting rules:**
    ...
    """
    ```
    This system prompt is the instruction manual for our LLM agent. It clearly defines its role, when to use tools, how to format tool calls, and how to behave in different scenarios. This detailed guidance is essential for the LLM to operate effectively.
    ...
    Based on the selected tool, it then *generates* the function name and arguments as structured outputs such as JSON or Pydantic. The LLM is specially tuned through instruction fine-tuning to interpret tool schema inputs and output tool calls. This fine-tuning allows the model to understand the intent behind a user's query and map it to the appropriate tool and its parameters [[56]](https://arxiv.org/html/2412.04503v1).
...
8.  We demonstrate how the `TOOL_CALLING_SYSTEM_PROMPT` works by calling the model with the first `USER_PROMPT`.
    ...
    ```
    Here, the LLM receives the user's request along with the tool definitions. It analyzes the request to determine if any of the provided tools are relevant.
...
9.  The LLM outputs the function's name and arguments, providing everything necessary to call the tool:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    The model correctly identifies that `search_google_drive` is the appropriate tool and extracts "latest quarterly report" as the query argument. This structured output is ready for programmatic execution.
...
10. We show how the `TOOL_CALLING_SYSTEM_PROMPT` works with another example by calling the model with a second `USER_PROMPT`.
    ...
    ```
    This second example demonstrates the LLM's ability to handle more complex, multi-step requests, even if it only suggests the first step initially.
...
11. The LLM's response again details the function call:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    Again, the LLM correctly identifies the initial tool call needed to fulfill the user's request.
...
12. Now, let's see how to interpret the output from the LLM. We start by extracting the tool call JSON string from the LLM response using the `extract_tool_call` function.
    ...
    def extract_tool_call(response_text: str) -> str:
        ...
        """
        response_text = response_text.replace("```tool_call", "<tool_call>").replace("```", "</tool_call>")
    ...
    tool_call_str
    ```
    This function is designed to parse the LLM's output, stripping away any surrounding text or markdown to isolate the pure JSON tool call.

13. We transform the JSON string into a Python dictionary:
    ```text
    '{"name": "search_google_drive", "args": {"query": "Q3 earnings report"}}'
    ```
    This step converts the stringified JSON into a usable Python dictionary, allowing us to access the tool's name and arguments programmatically.
...
14. We get the right tool handler from the `TOOLS_BY_NAME` dictionary to execute the actual function.
    ...
    tool_call
    ```
    This dictionary now contains the `name` of the tool to be called and the `args` to pass to it.

15. The tool handler is a reference to our `search_google_drive` function:
    ```text
    {'name': 'search_google_drive', 'args': {'query': 'Q3 earnings report'}}
    ```
    This confirms that we have successfully retrieved the Python function associated with the LLM's requested tool.
...
16. We call the tool with the extracted arguments.
    ...
    tool_handler
    ```
    The `tool_handler` is invoked with the arguments provided by the LLM, executing the simulated Google Drive search.
...
17. The output from the tool provides the mocked content of the financial report:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    This is the result of our mock tool execution, which would typically be real data fetched from an external system.
...
18. We define the `call_tool` function that aggregates all the steps from above: extract Python dictionary, get tool handler, and call the tools. Using it, we can directly pass the LLM response and get the tool output.
    ...
    ```
    This `call_tool` function streamlines the process, encapsulating the parsing and execution logic into a single, reusable function.
...
19. Calling the `call_tool` function shows the same output as before:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    This confirms the `call_tool` function works as expected, providing the tool's output.
...
20. Usually, we use an LLM to interpret the tool output. To do so, the tool result is sent back to the LLM to formulate a final response to the user or decide on the next step.
    ...
    ```
    After executing a tool, the LLM needs to process the result to provide a meaningful response to the user. This involves sending the tool's output back to the LLM as additional context.
...
21. The LLM response after it interprets the tool output summarizes the financial report:
    ...
    *   **Financial Health:** The company maintains a healthy cash flow position.
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    The LLM successfully interprets the tool's output and generates a concise, informative summary for the user. This completes a single turn of tool calling.

## Implementing a Small Tool Calling Framework From Scratch

Manually defining schemas for every tool we want to use can quickly become cumbersome and hard to scale. That's why all modern AI agent frameworks (e.g., LangGraph) or protocols (e.g., MCP) implement a `@tool` decorator (or something similar) that automatically computes and tracks the schemas of decorated functions. LangGraph is a framework for building stateful, multi-actor applications with LLMs, which we will explore in future lessons. MCP, or Multi-Agent Communication Protocol, is a standardized way to expose tools to agents, which we will cover in Part 2 of the course.
...
1.  We define the `ToolFunction` class. This class will aggregate the function schema and provide a callable wrapper for the original function. It acts as a container for both the function and its automatically generated schema.
    ```python
    ...
    ```
    This `ToolFunction` class allows us to treat our decorated Python functions as first-class tool objects, making them easily manageable within our framework.

2.  We define the `tool` function, which will be used to decorate future functions using the `@tool` syntax. Python decorators are functions that take another function as an argument and extend or modify the behavior of the latter without explicitly modifying it. Our `tool` decorator inspects the function's signature and docstring to automatically generate a JSON schema.
    ```python
    def tool(description: Optional[str] = None) -> Callable[[Callable], ToolFunction]:
        ...
        return decorator
    ```
    This decorator abstracts away the manual schema creation process, allowing developers to focus on writing function logic and clear docstrings. It automatically infers the tool's name, description, and argument structure, simplifying tool registration [[25]](https://python.langchain.com/docs/concepts/tools/).
...
3.  We redefine the three tools (`search_google_drive_example`, `send_discord_message_example`, `summarize_financial_report_example`) using the `@tool` Python decorator.
    ...
    ```
    By simply adding `@tool()` above each function, we transform them into tool objects with automatically generated schemas. This is a clean and efficient way to define tools.
...
4.  The `tools` registry now contains our decorated tools.
    ...
    ```
    This list of `ToolFunction` objects forms our tool library, ready to be presented to the LLM.

5.  The `search_google_drive_example` tool is now of type `__main__.ToolFunction`:
    ```text
    __main__.ToolFunction
    ```
    This output confirms that our decorator successfully wrapped the original Python function into our custom `ToolFunction` object.
...
6.  Inspecting the `search_google_drive_example` tool from the registry shows its name, type, schema, and functional handler. The schema is identical to the one we manually defined earlier:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    This detailed inspection shows that our decorator correctly extracted all the necessary metadata from the function's signature and docstring to build a valid JSON schema.

7.  The functional handler is a reference to our original Python function:
    ```text
    <function __main__.search_google_drive_example(query: str) -> dict>
    ```
    This confirms that the original function is still accessible and can be invoked when the LLM requests it.
...
8.  We call the LLM with the new `tools_schema` variable using the `USER_PROMPT` input.
    ...
    ```
    The LLM now receives the automatically generated schemas, simplifying the prompt construction.
...
9.  The LLM outputs the function call, as expected:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    The model successfully uses the automatically generated schema to suggest the correct tool call.
...
10. We call the tool from the LLM response using the `call_tool` function (defined in Section 3) and output its result.
    ...
    ```
    This demonstrates that our `call_tool` function works seamlessly with the tools defined using our new decorator.
...
11. The output shows the mocked search result:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    The tool execution is successful, returning the expected mock data. This confirms that our small tool calling framework, built from scratch using decorators, functions correctly. This implementation is similar to what LangGraph does, by using the `@tool` decorator [[25]](https://python.langchain.com/docs/concepts/tools/).

## Implementing Production-Level Tool Calls with Gemini

Similar to what we did in Lesson 4 on structured outputs, after writing the tool calling implementation from scratch, we want to show you how you can use modern APIs such as Gemini's to make your life easier and your code more robust. Rather than prompt engineering the LLM on what tools to pick and how to output tool calls in the right format, we will use Gemini's `GenerateContentConfig` to define all the available tools. This approach is more efficient and reliable, as the LLM provider optimizes tool calling for its specific models [[48]](https://codelabs.developers.google.com/codelabs/gemini-function-calling).

Now, let's dig into the code and see how we can use Gemini's `GenerateContentConfig` config to use multiple tools.

1.  We define the `tools` input and `config` object for Gemini. At this point, we still use the schemas defined above, but we will soon show how to skip this step by passing Python functions directly.
    ```python
    tools = [
        types.Tool(
    ...
    ```
    Here, we explicitly declare our tools using Gemini's `types.FunctionDeclaration` and `types.Tool` objects. This tells the Gemini API about the functions our agent can use.
...
2.  We call the LLM with the defined `config`. We can completely skip defining the huge `TOOL_CALLING_SYSTEM_PROMPT` system prompt that guides the LLM on how to use the tools and directly input the `USER_PROMPT`. This is more robust, as we are certain that the LLM is instructed the right way how to use the tools [[48]](https://codelabs.developers.google.com/codelabs/gemini-function-calling).
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    By using Gemini's native tool configuration, the API handles the internal prompt engineering, ensuring optimal performance and adherence to tool usage guidelines. This simplifies our application code significantly.
...
3.  The LLM's response returns a function call:
    ...
    function_call
    ```
    Gemini returns a `FunctionCall` object, which is a structured representation of the tool the model intends to invoke.

4.  The `function_call` object returned by Gemini is a structured object:
    ```text
    FunctionCall(id=None, args={'query': 'Q3 earnings report'}, name='search_google_drive')
    ```
    This output is consistent with the structured tool calls we implemented from scratch, but it is generated directly by the Gemini API.

5.  To simplify the implementation even more, and avoid computing the tool schemas manually or through a `@tool` decorator, we show how Google's `genai` Python SDK supports taking functions directly as input. We create a new `config` object by passing directly the `search_google_drive` and `send_discord_message` functions. The SDK automatically creates the schema based on the signature, type hints, and pydocs of each function, as we did so far with our from-scratch implementations [[52]](https://philschmid.de/gemini-function-calling).
    ```python
    config = types.GenerateContentConfig(
    ...
    function_call = response_message_part.function_call
    ```
    This feature of the `genai` SDK further streamlines tool integration, as it automatically handles the conversion of Python functions into `FunctionDeclaration` objects.

6.  The LLM response shows the function call:
    ```text
    FunctionCall(id=None, args={'query': 'Q3 earnings report'}, name='search_google_drive')
    ```
    The model's output remains consistent, demonstrating the flexibility of the SDK's automatic function declaration.
...
7.  The `function_call.args` returned by Gemini contains the arguments as a dictionary:
    ...
    tool_handler
    ```
    We extract the tool's name from the `function_call` object to retrieve its handler.

8.  We access the `tool_handler` based on the function's name from the `TOOLS_BY_NAME` registry, as before:
    ```text
    <function __main__.search_google_drive(query: str) -> dict>
    ```
    This confirms that we can still use our existing `TOOLS_BY_NAME` registry to map the LLM's suggested tool to our Python function.

9.  We call the `tool_handler` manually.
    ```python
    tool_handler(**function_call.args)
    ```
    The tool is executed with the arguments provided by the LLM.
...
10. The output is the mocked content of the financial report:
    ...
    {'files': [{'name': 'Q3_Earnings_Report_2024.pdf',
       ...
       'content': '\n# Q3 2023 Financial Performance Analysis\n\nThe Q3 earnings report shows a 20% increase in revenue and a 15% growth in user engagement, \nbeating market expectations. These impressive results reflect our successful product strategy \nand strong market positioning.\n\nOur core business segments demonstrated remarkable resilience, with digital services leading \nthe growth at 25% year-over-year. The expansion into new markets has proven particularly \nsuccessful, contributing to 30% of the total revenue increase.\n\nCustomer acquisition costs decreased by 10% while retention rates improved to 92%, \nmarking our best performance to date. These metrics, combined with our healthy cash flow \nposition, provide a strong foundation for continued growth into Q4 and beyond.\n'}]}
    ```
    This is the result of the tool's execution.
...
11. We call the `call_tool` function (defined in Section 3) to call the `function_call` from the LLM response in one go.
    ...
    ```
    This demonstrates the streamlined execution of tools using our `call_tool` helper.
...
Adding the Pydantic model as a tool is an elegant way to get structured outputs in agentic scenarios. In these scenarios, you often take multiple intermediate steps using unstructured outputs (which are easily interpreted by an LLM). Then, you dynamically decide when to output the final answer in a structured form as a Pydantic model. This Pydantic model is easily interpreted in Python downstream steps and ensures the final output has the expected schema [[33]](https://arxiv.org/html/2505.03049v2). This approach combines the flexibility of LLM reasoning with the robustness of structured data validation.

Image 3 illustrates an AI agent's workflow with a tool-calling loop and a conditional Pydantic tool for structured output. This diagram shows how an agent can dynamically decide between calling various tools for intermediate steps and, when appropriate, invoke a Pydantic-based tool to extract structured information as a final output.
...
1.  We define the `DocumentMetadata` Pydantic class, similar to what we did in Lesson 4. This class serves as our schema for the structured output we want to extract.
    ```python
    ...
    ```
    This Pydantic model provides a clear, type-hinted definition for the metadata we expect to extract from a financial document.
...
2.  We define the `extraction_tool` object. We manually define the function schema, using `DocumentMetadata.model_json_schema()` as parameters, similar to how we instructed the LLM to generate structured outputs in Lesson 4 [[35]](https://pydantic.dev/articles/llm-intro).
    ...
    ```
    By treating our Pydantic model's schema as the `parameters` for a `FunctionDeclaration`, we instruct the LLM to "call" this schema, effectively asking it to generate an output that conforms to our `DocumentMetadata` model.
...
3.  We define the `config` object, including our `extraction_tool`.
    ...
    ```
    This configuration tells the Gemini API that our LLM has access to an `extract_metadata` tool, whose output should conform to the `DocumentMetadata` schema.
...
4.  We define the `prompt` and call the LLM.
    ...
    response_message_part = response.candidates[0].content.parts[0]
    ```
    The LLM receives the document and the instruction to extract metadata, knowing it must return this information in the structured format defined by our Pydantic tool.
...
5.  The function name and arguments are output as the response from the LLM:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    The LLM successfully generates a `function_call` for `extract_metadata`, with arguments that match the fields of our `DocumentMetadata` Pydantic model.
...
6.  We create the `document_metadata` Pydantic object, which outputs `Validation successful!` if the output arguments have the right signature:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    By passing the LLM's generated arguments directly to our `DocumentMetadata` Pydantic model, we achieve automatic validation and conversion into a strongly typed Python object. This ensures data integrity and simplifies downstream processing. This approach is commonly used in AI agents that require structured outputs.
...
Image 4 illustrates a tool calling loop. This sequence shows how an LLM agent can iteratively call tools, process their results, and decide on subsequent actions until a task is completed.
...
```
Image 4: A sequence diagram illustrating a tool calling loop. (Source [Towards AI [60]](https://pub.towardsai.net/crafting-langchain-tools-a-complete-guide-to-custom-tool-development-f21fd2f16622))

The benefits of running tools in a loop are clear: flexibility, adaptability, and the ability to handle complex multi-step tasks. This allows agents to break down problems into smaller, manageable steps, executing each part with the appropriate tool. Now, let's dig into the code and see how we can implement tool calling in a loop.

1.  We define a new `config` object that includes all three mock tools. This configuration enables the LLM to choose from any of our defined tools during the multi-step process.
    ```python
    tools = [
        types.Tool(
            function_declarations=[
    ...
    ```
    This setup provides the LLM with a comprehensive set of tools for the task.

2.  We define the user intent through the `USER_PROMPT`. This prompt requires a sequence of actions to be completed.
    ```python
    USER_PROMPT = """
    Please find the Q3 earnings report on Google Drive and send a summary of it to 
    the #finance channel on Discord.
    ...
    ```
    The user's request clearly implies a multi-step process: search, summarize, and send.

3.  We implement the first LLM call. This initiates the tool-calling loop, with the LLM suggesting the first action.
    ```python
    pretty_print.wrapped(USER_PROMPT, title="User Prompt")
    response = client.models.generate_content(
        model=MODEL_ID,
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    The LLM correctly identifies the initial step: searching Google Drive for the report.

4.  We implement the sequential tool calling loop. This `while` loop continues as long as the LLM suggests further function calls or until a maximum number of iterations is reached.
    ```python
    ...
    ```
    This loop is the core of our agent's multi-step capability. It executes tools, feeds results back to the LLM, and allows the LLM to decide the next action based on the updated context.
...
5.  The outputs of the tool calling loop show the sequence of actions taken by the LLM:
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
    The outputs clearly show the sequence of tool calls: first `search_google_drive`, then `summarize_financial_report`, and finally `send_discord_message`. This demonstrates the agent's ability to chain actions to complete a complex task.

This approach, while powerful for multi-step tasks, has limitations [[14]](https://arxiv.org/pdf/2503.13657). It doesn't allow the LLM to interpret each tool output before deciding on the next steps. The agent immediately moves to the next function call without pausing to think about what it learned or whether it should change strategy. This can lead to inefficient tool usage or getting stuck in loops [[15]](https://arxiv.org/html/2412.01130v2). The LLM lacks explicit reasoning steps between actions, which can hinder its ability to adapt to unexpected outcomes or refine its plan dynamically [[42]](https://www.promptingguide.ai/techniques/react).
...
Here is a list of popular tools, grouped based on functionality:

1.  **Knowledge & Memory Access:**
    *   Tools that query vector databases, document stores, graph databases, or other knowledge bases to retrieve relevant context for the LLM. This is the core of Retrieval-Augmented Generation (RAG), a concept we will discuss in more detail in Lesson 10. RAG allows LLMs to access up-to-date and proprietary information, overcoming the static nature of their training data [[23]](https://learn.microsoft.com/en-us/azure/search/retrieval-augmented-generation-overview).
    *   We can take the idea even further and provide the LLM text-to-SQL tools that can construct SQL/NoSQL queries and interact with classic databases. This pattern is known as text-to-SQL. These tools enable LLMs to extract and analyze structured data, which is essential for business intelligence and reporting [[18]](https://mirascope.com/blog/llm-integration).
    *   All these tools are closely related to memory, RAG, and agentic RAG, which we will discuss in more detail in Lessons 9 (memory) and 10 (RAG). They provide the LLM with access to vast amounts of external knowledge, making its responses more accurate and contextually relevant [[22]](https://www.purestorage.com/knowledge/what-is-retrieval-augmented-generation.html).
2.  **Web Search & Browsing:**
    *   Tools that interface with search engine APIs (e.g., Google Search, Bing Search, Brave Search). These tools allow LLMs to fetch real-time web data, providing current information for user queries [[29]](https://www.business-standard.com/technology/tech-news/microsoft-brings-copilot-ai-powered-web-search-mode-on-bing-how-it-works-125022500477_1.html).
    *   Web scraping tools that can fetch and parse content from web pages. This enables agents to gather information from specific online sources that might not be indexed by general search engines.
    *   These types of tools are usually omnipresent in chatbots and research agents, allowing them to access up-to-date information [[30]](https://support.microsoft.com/en-us/topic/copilot-in-bing-our-approach-to-responsible-ai-45b5eae8-7466-43e1-ae98-b48f8ff8fd44). Examples include Bing Copilot and Perplexity AI, which integrate web search to provide current and fact-based answers [[31]](https://www.byteplus.com/en/topic/560528).
3.  **Code Execution:**
    *   For example, a Python interpreter tool allows the agent to write and execute Python code in a sandboxed environment. This is invaluable for calculations, data manipulation, statistics, and data visualizations [[12]](https://dida.do/blog/setting-up-a-secure-python-sandbox-for-llm-agents). It enables the agent to perform complex computational tasks that are beyond the LLM's inherent capabilities [[19]](https://arxiv.org/html/2506.18096v1).
    ...
    *   Using code execution tools introduces security risks like arbitrary code execution, resource exhaustion, and unauthorized file system access, necessitating sandboxing and isolation strategies [[39]](https://dida.do/blog/setting-up-a-secure-python-sandbox-for-llm-agents). OWASP highlights prompt injection and insecure output handling as key risks, recommending rigorous output validation and sandboxed environments [[13]](https://owasp.org/www-project-top-10-for-large-language-model-applications/).
4.  **Other Popular Tools:**
    *   Interacting with external APIs (e.g., calendar, email, project management, calls) are omnipresent in enterprise AI applications. These integrations automate workflows and enhance productivity by allowing LLMs to trigger actions in business systems [[16]](https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models).
    *   File system operations (read/write files, list directories) are omnipresent in productivity AI applications that have to interact with our operating system. This enables agents to manage and manipulate local files as part of their tasks.
