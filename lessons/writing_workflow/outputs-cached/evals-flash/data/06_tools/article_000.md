...
Now, we are ready to tackle one of the most transformative aspects of AI agents: **tool usage**. Tools are what empower an LLM to go beyond generating text and interact with the external world. They are a foundational block of any AI agent, enabling it to perform actions and access real-time information. Understanding how an LLM decides which tool to call, generates the correct parameters, and executes functions is critical for building, debugging, and monitoring robust AI applications. In this lesson, we will implement tool calling from scratch, leverage modern APIs like Gemini, explore using Pydantic models as tools, and discuss the limitations that lead to more sophisticated patterns like ReAct. We will also examine the most essential tool categories, from memory access to web search and code execution.
...
This is where tools kick in. Think of the LLM as the brain, and tools as its "hands and senses." Tools are the bridge between the LLM's internal reasoning and the dynamic external world. With tools, an LLM becomes an AI agent that can perceive its environment, execute specific instructions, and perform actions.
...
Image 1: High-level architecture of an LLM acting as an agent with tool use.

As seen in Image 1, the LLM processes user requests and, if an external action is needed, decides which tool to use. This decision is then passed to an integrator, which executes the tool via a tool execution pipeline. The result is parsed and fed back to the LLM, allowing it to formulate a coherent response or take further steps [[18]](https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models).

Modern AI agents leverage various tools to extend their capabilities. Some popular examples include:
*   **Accessing real-time information:** Tools that query APIs for current weather, news, or stock prices [[19]](https://aman.ai/primers/ai/agents/).
*   **Interacting with databases:** Tools that query structured databases (PostgreSQL) or data lakes (S3) to retrieve or analyze information [[20]](https://mirascope.com/blog/llm-integration).
*   **Accessing long-term memory:** Tools that retrieve information from an agent's memory beyond its current context window, a concept we will explore in Lesson 9 on Agent Memory & Knowledge [[27]](https://www.weka.io/learn/guide/ai-ml/retrieval-augmented-generation/).
*   **Executing code:** A Python interpreter tool allows the agent to write and run code in a sandboxed environment for calculations, data manipulation, or visualizations [[21]](https://arxiv.org/html/2506.18096v1).
*   **Performing precise calculations:** Tools that handle mathematical operations, sorting, filtering, or grouping data, going beyond the LLM's inherent computational limitations [[45]](https://www.projectpro.io/article/llm-limitations/1045).

These tools enable LLMs to overcome their static nature and perform dynamic, real-world tasks, transforming them into powerful AI agents [[47]](https://memgraph.com/blog/llm-limitations-query-enterprise-data).
...
sequenceDiagram
    ...
    LLM-->>Application: "3. Responds with function_call (tool name, args)"
...
```
Image 2: A sequence diagram illustrating the 6 steps of a tool calling process.
...
2.  We define three mock tools: `search_google_drive`, `send_discord_message`, and `summarize_financial_report`. To keep the code simple and focus on the tools implementation, all tools are mocked.
    ```python
...
7.  We define and explain the `TOOL_CALLING_SYSTEM_PROMPT`. This prompt guides the LLM on how and when to use the tools. It includes tool usage guidelines, the expected tool call format, response behavior, and a list of available tools enclosed by XML tags.
    ...
    **Critical formatting rules:**
    ...
    """
    ```
    Based on the `description` field from the tool schema, the LLM *decides* if a tool call is appropriate to fulfill the user query. That's why writing clear and articulate tool descriptions is extremely important for building successful AI agents. When passing multiple tools, it is critical for the descriptions to distinguish between each other to avoid any future confusion. For example, two tools with descriptions "Tool used to search documents" and "Tool used to search files" would confuse the LLM. You have to be as explicit as possible and define them as "Tool used to search documents on Google Drive" and "Tool used to search files on the disk."
    
    Another disambiguation method when working with AI agents is to be as clear as possible in the system prompts, verbosely stating what you need. For example, instead of saying "search documents," be explicit and say where to search them from, such as "search documents on Google Drive." By defining clear tool descriptions and system prompts, you ensure the AI agent will be able to make the necessary matches and call the right tools. This becomes crucial when scaling up to 50-100 tools per AI Agent. We will dig more into scaling methods in Part 2 and 3 of the course.
...
10. We call the tool from the LLM response using the `call_tool` function and output its result.
    ```python
    pretty_print.wrapped(
        json.dumps(call_tool(response.text, tools_by_name=tools_by_name), indent=2), title="LLM Tool Call Response"
    )
    ...
    ---------------------------------------------------------------------------------------------------- 
    ```
Voilà! We have our little tool calling framework. This implementation is similar to what LangGraph does under the scenes, by leveraging the `@tool` decorator [[32]](https://python.langchain.com/docs/concepts/tools/).

## Implementing Production-Level Tool Calls with Gemini

After writing the tool calling implementation from scratch, we want to show you how you can leverage modern APIs such as Gemini's to make your life easier and your code more robust. Rather than prompt engineering the LLM on what tools to pick and how to output tool calls in the right format, we will leverage Gemini's `GenerateContentConfig` to define all the available tools.
...
5.  The `function_call.args` returned by Gemini contains the arguments as a dictionary.
    ```python
    tool_handler = TOOLS_BY_NAME[function_call.name]
    tool_handler
    ```

6.  We access the `tool_handler` based on the function's name from the `TOOLS_BY_NAME` registry, as before.
    ```text
    <function __main__.search_google_drive(query: str) -> dict>
    ```

7.  We call the `tool_handler` manually.
    ```python
    tool_handler(**function_call.args)
    ```

8.  The output is the mocked content of the financial report.
    ```text
...
9.  We explain the `call_tool` implementation.
    ```python
...
10. We call the `call_tool` function to call the `function_call` from the LLM response in one go.
    ```python
...
2.  We define the `extraction_tool` object. We manually define the function schema, leveraging `DocumentMetadata.model_json_schema()` as parameters, similar to how we instructed the LLM to generate structured outputs in Lesson 4 [[138]](https://www.deeplearning.ai/short-courses/pydantic-for-llm-workflows/).
    ```python
...
The benefits of running tools in a loop are clear: flexibility, adaptability, and the ability to handle complex multi-step tasks. Now, let's dig into the code and see how we can define Pydantic structured outputs as tools.
...
2.  **Web Search & Browsing:**
    ...
    *   These types of tools are usually omnipresent in chatbots and research agents, allowing them to access up-to-date information [[37]](https://gaper.io/perplexity-ai-vs-google-gemini-vs-chatgpt/).
3.  **Code Execution:**
    ...
    *   Using code execution tools introduces security risks like arbitrary code execution and resource exhaustion, necessitating sandboxing and isolation strategies [[72]](https://www.legitsecurity.com/aspm-knowledge-base/llm-security-risks).
...
These diverse tool categories highlight the expansive capabilities that tools bring to LLM agents, transforming them into versatile problem-solvers across various domains [[64]](https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models).

## Conclusion

Tool calling sits at the core of AI agents and is probably the most important skill you need to deeply understand how to build, monitor, and debug AI applications. By enabling LLMs to interact with external systems, tools transform them from mere text generators into powerful, action-oriented agents. We have explored this transformation from scratch, implemented frameworks, leveraged native API capabilities, and even used Pydantic models to ensure structured outputs.

As we move forward, we will build upon this foundation. In Lesson 7, we will dive into the theory behind planning and ReAct agents, addressing the limitations of simple tool-calling loops. You will learn about advanced retrieval techniques for knowledge-augmented agents in Lesson 10 on RAG Deep Dive, and how to work with documents, images, and complex data in Lesson 11 on Multimodal Processing. Part 2 of the course will then guide you through building a complete research and writing agent system, while Part 3 focuses on production-grade engineering practices. Our goal is to equip you with the knowledge to ship AI products that work, moving you from the PoC purgatory to real-world impact.
...
3.  Tool Calling Agent From Scratch. (n.d.). YouTube. https://www.youtube.com/watch?v=ApoDzZP8_ck
4.  Efficient Tool Use with Chain-of-Abstraction Reasoning. (n.d.). arXiv. https://arxiv.org/pdf/2401.17464v3
5.  Building AI Agents from scratch - Part 1: Tool use. (n.d.). Swirl AI. https://www.newsletter.swirlai.com/p/building-ai-agents-from-scratch-part
6.  What is Tool Calling? Connecting LLMs to Your Data. (n.d.). YouTube. https://www.youtube.com/watch?v=h8gMhXYAv1k
7.  ReAct vs Plan-and-Execute: A Practical Comparison of LLM Agent Patterns. (n.d.). DEV Community. https://dev.to/jamesli/react-vs-plan-and-execute-a-practical-comparison-of-llm-agent-patterns-4gh9
8.  Agentic Design Patterns Part 3, Tool Use. (n.d.). DeepLearning.AI. https://www.deeplearning.ai/the-batch/agentic-design-patterns-part-3-tool-use/
9.  Comprehensive Guide to Integrating Tools and APIs with Language Models. (n.d.). Mercity.ai. https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models
10. Introduction. (n.d.). Xebia. https://xebia.com/blog/enforce-and-validate-llm-output-with-pydantic/
11. Introduction. (n.d.). LeewayHertz. https://www.leewayhertz.com/structured-outputs-in-llms/
12. (n.d.). Dida.do. https://dida.do/blog/setting-up-a-secure-python-sandbox-for-llm-agents
13. (n.d.). OWASP. https://owasp.org/www-project-top-10-for-large-language-model-applications/
14. (n.d.). arXiv. https://arxiv.org/pdf/2503.13657
15. (n.d.). arXiv. https://arxiv.org/html/2412.01130v2
16. (n.d.). Mercity.ai. https://www.mercity.ai/blog-post/guide-to-integrating-tools-and-apis-with-language-models
17. (n.d.). Aman.ai. https://aman.ai/primers/ai/agents/
18. (n.d.). Mirascope. https://mirascope.com/blog/llm-integration
19. (n.d.). arXiv. https://arxiv.org/html/2506.18096v1
20. (n.d.). Weka.io. https://www.weka.io/learn/guide/ai-ml/retrieval-augmented-generation/
21. (n.d.). HKEX News. https://www1.hkexnews.hk/app/sehk/2025/107494/documents/sehk25062703133.pdf
22. (n.d.). Pure Storage. https://www.purestorage.com/knowledge/what-is-retrieval-augmented-generation.html
23. (n.d.). Microsoft Learn. https://learn.microsoft.com/en-us/azure/search/retrieval-augmented-generation-overview
24. (n.d.). K2View. https://www.k2view.com/what-is-retrieval-augmented-generation
25. (n.d.). LangChain. https://python.langchain.com/docs/concepts/tools/
26. (n.d.). ITNext. https://itnext.io/the-mcp-revolution-transforming-agents-with-mcp-2f053da01e8c
27. (n.d.). LangChain. https://python.langchain.com/docs/how_to/custom_tools/
28. (n.d.). LangGraph. https://langchain-ai.github.io/langgraph/how-tos/tool-calling/
29. (n.d.). Business Standard. https://www.business-standard.com/technology/tech-news/microsoft-brings-copilot-ai-powered-web-search-mode-on-bing-how-it-works-125022500477_1.html
30. (n.d.). Microsoft Support. https://support.microsoft.com/en-us/topic/copilot-in-bing-our-approach-to-responsible-ai-45b5eae8-7466-43e1-ae98-b48f8ff8fd44
31. (n.d.). BytePlus. https://www.byteplus.com/en/topic/560528
32. (n.d.). Pydantic. https://pydantic.dev/articles/llm-validation
33. (n.d.). arXiv. https://arxiv.org/html/2505.03049v2
34. (n.d.). Dylan Castillo. https://dylancastillo.co/posts/agentic-workflows-langgraph.html
35. (n.d.). Pydantic. https://pydantic.dev/articles/llm-intro
36. (n.d.). ProjectPro. https://www.projectpro.io/article/llm-limitations/1045
37. (n.d.). arXiv. https://arxiv.org/html/2504.14872v1
38. (n.d.). EDPB. https://www.edpb.europa.eu/system/files/2025-04/ai-privacy-risks-and-mitigations-in-llms.pdf
39. (n.d.). Dida.do. https://dida.do/blog/setting-up-a-secure-python-sandbox-for-llm-agents
40. (n.d.). Hugging Face. https://huggingface.co/docs/smolagents/en/tutorials/secure_code_execution
41. (n.d.). Amir Malik. https://amirmalik.net/2025/03/07/code-sandboxes-for-llm-ai-agents
42. (n.d.). PromptingGuide.ai. https://www.promptingguide.ai/techniques/react
43. (n.d.). Cobalt.io. https://www.cobalt.io/blog/llm-failures-large-language-model-security-risks
44. (n.d.). Exabeam. https://www.exabeam.com/explainers/ai-cyber-security/llm-security-top-10-risks-and-7-security-best-practices/
45. (n.d.). ISC.upenn.edu. https://isc.upenn.edu/security/LLM-guide
46. (n.d.). arXiv. https://arxiv.org/html/2507.08034v1
47. (n.d.). GetDynamiq.ai. https://www.getdynamiq.ai/post/llm-agents-explained-complete-guide-in-2025
48. (n.d.). Google Codelabs. https://codelabs.developers.google.com/codelabs/gemini-function-calling
49. (n.d.). Google Cloud. https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/function-calling
50. (n.d.). Google AI for Developers. https://ai.google.dev/gemini-api/docs/function-calling
51. (n.d.). Firebase. https://firebase.google.com/docs/ai-logic/function-calling
52. (n.d.). YouTube. https://www.youtube.com/watch?v=mVXrdvXplj0
53. (n.d.). GDELT Project. https://blog.gdeltproject.org/large-language-models-llms-planetary-scale-realtime-data-current-limitations/
54. (n.d.). MIT Press. https://direct.mit.edu/opmi/article/doi/10.1162/opmi_a_00160/124234/The-Limitations-of-Large-Language-Models-for
55. (n.d.). arXiv. https://arxiv.org/html/2412.04503v1
56. (n.d.). Stanford University. https://uit.stanford.edu/service/techtraining/ai-demystified/llm
57. (n.d.). Decodable.co. https://www.decodable.co/blog/llms-need-real-time-data-to-deliver-contextual-results
58. (n.d.). LearnPrompting.org. https://learnprompting.org/docs/basics/pitfalls
59. (n.d.). Atamel.dev. https://atamel.dev/posts/2024/08-06_deepdive_function_calling_gemini/
60. (n.d.). Google AI for Developers. https://discuss.ai.google.dev/t/function-calling-multiple-function-results/60678
61. (n.d.). Towards AI. https://pub.towardsai.net/crafting-langchain-tools-a-complete-guide-to-custom-tool-development-f21fd2f16622
62. (n.d.). Pydantic. https://docs.pydantic.dev/latest/concepts/models/
63. (n.d.). Neo4j. https://neo4j.com/blog/developer/agentic-graphrag-for-commercial-contracts/
64. (n.d.). IBM Technology. https://www.youtube.com/watch?v=h8gMhXYAv1k
65. (n.d.). X.ai. https://docs.x.ai/docs/guides/function-calling
66. (n.d.). Chris Toolivier. https://blog.christoolivier.com/p/llms-and-functiontool-calling
67. (n.d.). PromptingGuide.ai. https://www.promptingguide.ai/applications/function_calling
68. (n.d.). OpenAI. https://platform.openai.com/docs/guides/function-calling
69. (n.d.). Friendli.ai. https://friendli.ai/blog/llm-function-calling
70. (n.d.). OpenAI Community. https://community.openai.com/t/how-does-function-calling-actually-work-for-the-assistants-api/641440
71. (n.d.). Apideck. https://www.apideck.com/blog/llm-tool-use-and-function-calling