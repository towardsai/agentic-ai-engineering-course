...
This is a critical failure mode for complex tasks. The agent produces a weak, unreliable output because it cannot break the problem down into logical sub-goals, such as verifying sources or comparing claims. This is often referred to as the "black box problem," where the model's internal decision-making is opaque, making it impossible to debug or trust for high-stakes tasks. The model might arrive at an answer, but we have no insight into *how* it got there, preventing any form of procedural verification. This lack of transparency is a major hurdle for enterprise adoption.

In previous lessons, we saw how workflows and structured outputs provide modularity, and how tools give an agent the ability to act [[1]](https://cdn.openai.com/business-guides-and-resources/a-practical-guide-to-building-agents.pdf). However, these components are just building blocks. Without an explicit reasoning process to guide them, the agent remains a brittle, linear system that can't adapt to unforeseen results. To fix this, we first need to teach the model how to think before it acts.
...
This is a significant improvement because the model is now creating an explicit plan [[3]](https://www.ibm.com/think/topics/ai-agent-planning). However, CoT has a major limitation: its computational cost. Generating a long reasoning chain requires significantly more tokens and processing power than a direct answer, which can be a barrier for real-time applications or systems operating on a tight budget. To gain the control and efficiency needed for a true agent, we have to separate the reasoning and execution processes.
...
## ReAct in Depth: The Loop of Thought and Observation
...
3.  **Observation:** The search tool returns a list of five academic papers. The abstract for Paper A mentions a 40% adoption rate for a specific edge AI technique, while Paper B's abstract mentions a 25% adoption rate for a similar technique.
4.  **Thought:** "I have detected a conflict in the reported adoption rates between Paper A and Paper B. This is a key finding that needs to be resolved. I need to find a third-party market analysis report to verify these figures before I can write my summary."
5.  **Action:** `search("edge AI deployment adoption market analysis 2024")`
6.  **Observation:** The search returns a link to a credible industry report.
7.  **Thought:** "The first three papers seem most relevant based on their titles. I need to extract their full text to understand their core findings. Paper A is from a top conference, Paper B has many citations, and Paper C is very recent."
8.  **Action:** `fetch_and_extract(["url_A", "url_B", "url_C"])`
9.  **Observation:** The tool returns the full text for the three papers.
...
However, this flexibility comes at a cost. ReAct can be slower and more expensive, as it often requires more LLM calls—one for each "Thought" step [[7]](https://dev.to/jamesli/react-vs-plan-and-execute-a-practical-comparison-of-llm-agent-patterns-4gh9). The process can also be difficult to implement in common programming languages like Python due to its complex state management requirements.

## Plan-and-Execute in Depth: Structuring the Path from Goal to Action
...
The main advantage of Plan-and-Execute is its ability to foster model creativity. By generating the full plan upfront, it allows the LLM to think holistically about the problem, which can lead to more novel and insightful solutions. It is also well-suited for artistic or generative tasks where a structured, predictable path is less important than a comprehensive initial vision.
...
An even more advanced capability is **interleaved thinking**, where the model can update its plan after each action within a single turn [[13]](https://docs.anthropic.com/en/docs/build-with-claude/extended-thinking#interleaved-thinking). This effectively bakes a ReAct-like loop directly into the model's inference process. These advancements make it easier to build simple agents.
...
**Goal decomposition** is the process of breaking a large, complex task into a hierarchy of smaller, more manageable sub-goals [[14]](https://www.amazon.science/blog/how-task-decomposition-and-smaller-llms-can-make-ai-more-affordable). In a Plan-and-Execute agent, this is the primary job of the planner. In a ReAct agent, decomposition happens more dynamically during the "Thought" steps, where the agent decides on the immediate next sub-goal. Effective prompting can guide the agent to perform this decomposition more robustly, ensuring that no critical steps are missed.

**Self-correction** is arguably the most critical capability for a robust agent. It's the ability to detect errors, contradictions, or low-confidence results during execution and dynamically update the plan to address them [[15]](https://galileo.ai/blog/self-evaluation-ai-agents-performance-reasoning-reflection). This is where the iterative nature of agentic loops truly shines. Imagine an agent tasked with booking a flight from New York to San Francisco. It initially finds a flight from JFK to SFO. However, during a verification step, it observes that the user's calendar shows a meeting in New Jersey just before the flight. A self-correcting agent recognizes this conflict. It inserts a new sub-goal: "Find a flight from an airport closer to New Jersey." It re-runs the search with Newark (EWR) as the origin, finds a more convenient option, and revises its plan.
