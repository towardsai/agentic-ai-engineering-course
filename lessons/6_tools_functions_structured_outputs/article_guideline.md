...
- Define Function Calling (focus on OpenAI's implementation as a prominent example, but generalize the concept).
...
- Show a simple code example using OpenAI's API to demonstrate defining a tool and handling a function call response.

### Section 3: Reliable Data Extraction with OpenAI's Structured Outputs

- Introduce OpenAI's Structured Outputs feature (e.g., JSON mode, or response_format={ "type": "json_object" }).
...
If the lesson aims to build this intuition:
...
- Purpose: To demystify what features like OpenAI's function calling abstract away and to appreciate their robustness and convenience.
